...
$A(t)$, also taken from \citet[eqs.~21-23]{MosterNaab+:2013a}. We
assume that accreted systems will have the same star formation history
as the host up until the moment of accretion. To find the total mass
(energy) injection including the contributions of accreted stars we
convolve the specific mass (energy) injection rate with the accretion
history $A(t)$.  The total mass ($\Mdot'(t)$) and energy
($\dot{E}'(t)$) rates will be given by
