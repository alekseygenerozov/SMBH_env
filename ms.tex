...
expected mass loss rates.  Solutions found to be unstable for all
values of $\eta$ are marked in boldface as ``TI" in Table
\ref{table:models}, with the unstable values of $\eta$ provided in the
final column.

Thermal conduction, by contrast, results in only order unity changes
to our results for the stagnation radius (and hence the accretion
rate) for our fiducial value of the saturation parameter $\phi =0.1$.
% Conduction may, however, offset the effects of radiative cooling in
% making the flow more thermally stable
% (e.g.~\citealt{Zakamska&Narayan03}).
