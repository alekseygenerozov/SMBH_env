...
\item Heating from SMBH feedback, unlike stellar heating, depends on the SMBH accretion rate, which is itself a function of the heating rate.  Compton heating is generally unimportant compared to stellar wind heating for the average SFH, but it can dominate in the case of an impulsive star burst (Fig.~\ref{fig:vwSourcesImp}).  Although in principle Compton heating is sufficiently high to prevent thermal instabilities when acting alone, i.e. $\dot{M}_{C} \lesssim \dot{M}_{\rm TI}$ (Fig.~\ref{fig:bh_growth}), SMBH feedback may not however be capable of truly stabilizing the flow given its inability to respond instantaneously to local changes in the properties of the gas.  Kinetic feedback could also be important in determining $\dot{M}(M_{\bullet}$) when stellar heating is weak ($\S\ref{sec:kinetic}$) but is more challenging to quantify.
