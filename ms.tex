...
\begin{figure}
  ...
  \caption{\label{fig:stag} \emph{Top panel:} Stagnation radius
    ...
    analytic prediction from equation (\ref{eq:rs2main}), with solid
    and dashed curves calculated for parameters $\{\Gamma=0.8, n=1\}$
    and $\{\Gamma=0.1,n=0.5\}$, respectively.  {\bf BDM: is the x axis
      here $v_{w}$ or $\vw$ !!?  fold $\eta$ into the y axis. AG: The
      axis here is $v_{w}$--it does not include the contribution from
      sigma. Should it be changed}}
\end{figure}
