...
\subsection{Nuclear X-ray Luminosities of Quiescent Galaxies}
\label{sec:mdot}

Figure \ref{fig:vwSources} shows that the CNM heating rate is on
average approximately constant with SMBH mass, except for the largest
black holes with $M_{\bullet} \gtrsim 5\times 10^{8}M_{\odot}$.  For
fixed wind heating $\dot{M}/\dot{M}_{\rm edd}$ increases with SMBH
mass $\propto M_{\bullet}^{0.5(0.8)}$ for core(cusp) galaxies,
respectively (Fig.~\ref{fig:mdot_mass}; eq.~[\ref{eq:eddr_analytic}]).
This appears in contradiction with the nuclear X-ray luminosities of
quiescent galaxies, which (though showing broad scatter) instead on
average obey $\langle L_X/L_{\rm edd}\rangle \sim \Mbh^\alpha$, where
$\alpha\simeq -0.25$ \citep{Pellegrini10, Miller+15}.
...
detection threshold (gray line).  Finally, shown with purple and
orange lines are the maximum accretion rates for thermally stable
accretion (eq.~[\ref{eq:Mdotmax}]) and set by Ia blow-out
(eq.~[\ref{eq:eddr_Ia}]), as well as the maximum accretion rate
allowed by Compton heating (eq.~[\ref{eq:MdotC}]).

For either of our assumptions about radiative efficiency our predicted
trend in $\langle L_{X}/L_{\rm edd}\rangle$ for core and cusp galaxies
is opposite the observationally inferred one.  However, the observed
