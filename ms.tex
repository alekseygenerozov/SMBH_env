...
emission (e.g.~\citealt{Narayan&Yi95}).  We caveat that, unlike stellar wind heating, Compton heating depends on radius as $v_{w}^{\rm C}(r) \propto (n^{2}\xi/\rho_{\star})^{1/2} \propto r^{(\Gamma-n-1)/2}$, i.e. $\propto r^{-0.6}$ and $\propto r^{-0.7}$ for core ($\Gamma = 0.8$; n = 1) and cusp ($\Gamma = 0.1$; n = 0.5) galaxies, respectively.  Thus, although the assumption of our numerical calculations that heating is radially constant is not satisfied, this radial dependence is sufficiently weak that we do not expect its presence to significantly alter our results.  
...
&, \text{cusp},
  ...
  \label{eq:vC2}
\end{align} 
The fact that $v_{w}^{\rm C} \gtrsim 300$ km s$^{-1}$ for physically realized values of $\eta \gtrsim 1$ shows that Compton heating can be significant in relatively young stellar populations with relatively high mass loss rates.  
...
\epsilon_{\rm j} \dot{M}c^{2}$, where $\epsilon_{\rm j} < 1$ is a jet efficiency factor, and that this energy is deposited as heat uniformly interior to a radius $r_{\rm heat}$ and volume $V_{\rm heat} \propto r_{\rm
...
Indeed, using the results of \citet{Bromberg+11}, we estimate that a jet of luminosity $L_{\rm
  j}$ and half opening angle $\theta_{\rm j} = 0.1$ (characteristic of
AGN jets) will propagate through a gaseous mass $M_{\rm g}$ to a radius $r$ in a timescale
\begin{eqnarray}
...
\end{eqnarray}
If we write $M_{\rm g} \sim \dot{M}t_{\rm ff}$ then the ratio of the jet crossing time to the dynamical in the bulge $t_{\rm dyn} \sim r/\sigma$ is given by
\begin{equation}
\frac{t_{\rm jet}}{t_{\rm dyn}} \sim 7\times 10^{-3}M_{\bullet,8}^{0.13}\left(\frac{\epsilon_{\rm j}}{10^{-6}}\right)^{-1/3},
\end{equation}
independent of radius $r$.  Thus a jet with sufficient power, $\epsilon_{j} \gtrsim 10^{-6}$, to appreciably heat the CNM on radial scales $\sim r_{\rm s}$, also has sufficient power to escape the gas within the nucleus and propogate to much larger radii.  Slower outflows from the accretion disk, instead of a collimated relativistic jet, may provide a more promising source of feedback in these systems (e.g.~\citealt{Li+13}).    
