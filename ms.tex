...
Figure \ref{fig:vwSources} shows that the average CNM heating rate is predicted to be approximately constant with SMBH mass, except for the largest black holes with $M_{\bullet} \gtrsim 5\times 10^{8}M_{\odot}$.  For
a fixed wind heating parameter, $\dot{M}/\dot{M}_{\rm edd}$ increases with SMBH
mass $\propto M_{\bullet}^{0.5(0.8)}$ for core(cusp) galaxies,
respectively (Fig.~\ref{fig:mdot_mass}; eq.~[\ref{eq:eddr_analytic}]).
This dependence appears in contradiction with the nuclear X-ray luminosities of
...
Figure~\ref{fig:miller} shows our predicted value of $\langle
L_{X}/L_{\rm edd} \rangle =\epsilon_X \Mdot c^2$ as a function of
black hole mass for core (blue) and cusp (green) galaxies.  The left
panel assumes a constant low radiative efficiency $\epsilon_X =
...
that depends on the Eddington ratio in the way predicted by simple
models for radiatively inefficient accretion flows
(\citealt{Narayan&Yi95}).  Superimposed with the theoretical curves
are symbols showing X-ray measurements (red stars) and upper limits
(red triangles) from \citet{Miller+15}, along with the best power-law
fit (red line; {\bf BDM: change to this!}) and the approximate X-ray
detection threshold (gray line).  Finally, shown with purple, orange, and {\bf brown} lines
orange lines are the maximum accretion rates for thermally stable
accretion (eq.~[\ref{eq:Mdotmax}]), set by SN Ia blow-out
(eq.~[\ref{eq:eddr_Ia}]), and allowed by Compton heating feedback (eq.~[\ref{eq:MdotC}]), respectively.

First note that to first order, our predicted values of $\langle L_{X}/L_{\rm edd}\rangle$ for heating corresponding to average star formation reside reasonably close to the data $\langle L_{X}/L_{\rm edd}\rangle$.  However, for either assumption about radiative efficiency, our predicted positive trend of $\langle L_{X}/L_{\rm edd}\rangle$ with black hole mass is opposite the observationally inferred one, which instead shows $L_{X}/L_{\rm edd}$ decreasing with $M_{\bullet}$.   Comparison to the data is improved, however, by taking into account the fact that larger(smaller) black hole masses are more commonly associated with core(cusp) galaxies, resulting in a flatter predicted trend for $\langle L_X/L_{\rm edd}\rangle$ when both samples are combined.   The large scatter observed in $L_{X}/L_{\rm edd}$ of typically two to four orders of magnitude in the \citet{Pellegrini10} sample can also be understood in our steady-accreting model by the .  Alternatively, cases where the older stellar population precludes a steady accretion flow, the scatter could be due to (e.g.~\citealt{Pellegrini&Ciotti98}; refs).  

   Finally, we note that \citet{Pellegrini10} finds that $L_{X}$ does not show any dependence on the age of the stellar population in the central galactic region for old populations with stellar ages $> 3$ Gyr, although less X-ray luminous nuclei are found among galaxies with a younger stellar population.  This 

 however, when the 

.  However, this conclusion comes with several caveats: (1) 
