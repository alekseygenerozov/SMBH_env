...
\begin{tabular}{lccccccccc}
  \hline
  {$M_{\bullet}$} & {$v_{w}$} & {$r_b^{(b)}$} &  $\Gamma$ &  $r_{\rm s}/r_{\rm soi}$ &  {$\left(\frac{\dot{M}}{\dot{M}_{\rm edd}}\frac{0.2}{\eta}\right)_{r_{s}}^{(c)}$} & {$\left(\frac{t_{\rm cool}}{t_{\rm ff}}\frac{0.2}{\eta}\right)_{r_s}^{(d)}$}  \\
  ($M_{\odot}$) & (km s$^{-1}$) & (pc) &- & - & - &  & \\ 
\hline
...
$    10^{ 6 }$ & 600 & 0.1 & 25 & $ 9.9 \times 10^{ -2 }$ & $ 8.2 \times 10^{ -6 }$ & 99 \\
...
In addition to our standard set of solutions neglecting thermal
conduction and cooling, we explore a few test simulations including
these effects, as marked in Table \ref{table:models}. For solutions we
with large wind heating parameters ($\vwO \gsim 1200 km/s$ km/s), we
expect radiative cooling to have virtually no effect on the solution
for reasonable values of the mass return parameter.  For cusp galaxies
Thermally stable solutions are possible for values of the wind heating
parameter as low as $\vwO=300$ km/s. However, this will be dependent
on the value of the mass return parameter (and thus on the age of the
stellar population) and on the break radius, $\rb$. For $\vwO\simeq
300$ km/s, solutions with large $\rb$ will have large stagnation radii
and thus larger densities. Thus, solution with $v_w$=300 km/s
$\rb\gsim 100$ pc, would be thermally unstable.  The picture is
qualitatively similar for core galaxies. However, the stagnation
radius ``runs away'' to large radii at much smaller values of the wind
heating paramter ($\vwO\simeq 600 km/s$). Thus, thermal instability
sets in for higher values of $\vwO$ for core galaxies.

In practice we expect that the effective wind heating will
be or order $\sim 1000$ km/s if heating is dominated by young
stars as would be the case for small black holes. This region of
parameter space should be thermally stable. For higher mass
black holes heating would be dominated by the supernovae and black
hole feedback and would be comparable to the stellar velocity
dispersion of the galaxy. Since core galaxies dominate for large
$\Mbh$ we expect this region of parameter space to be thermally unstable.

From our analytic results (see equation~\eqref{eq:conduction}) and
direct numerical experiments, we expect that thermal conduction would
introduce at most order unity changes in our solutions. The conductive
heating flux saturates, so that at the stagnation radius the
conductive heating rate is comparable to the external heating rate.
...
black hole feedback. We compute profile for a range of different black
hole masses, heating rates, and stellar density profiles and use these
profiles to calculate an $\Mdot$ onto each galaxy's SMBH, and get the
scaling of $\Mdot$ with $\Mbh$. We may then compare our results with
observed trends of Our main conclusions may be summarized as follows.

  \begin{enumerate}
  %Younger population=>higher accretion rate; generally higher eta and
  %lower vw except for very young stellar populations.
  \item We find that $\eddr \propto \Mbh^{}$ for
    $\Mbh\gsim4\times10^6$ and $\Mbh\lsim 2\times 10^8$, for galaxies
    following the average star formation histories measured in
