...
\epsilon_{\rm rad}/0.01 \sim 1$ is the maximum radiative efficiency for a thick
accretion flow (e.g.~\citealt{Narayan&Yi95}).  If heating
were dominated by Compton heating, i.e. $\tilde{v}_{w} = v_{w}^{C}$, then equation (\ref{eq:vC}) can be solved for $\tilde{v}_{\rm w}$ to find
\begin{align} v_{w}^{\rm C} \simeq
...
&, \text{cusp},
  ...
  \label{eq:vC2}
\end{align} 
The fact that $v_{w}^{\rm C} \lesssim 200$ km s$^{-1}$ for $\eta \lesssim 1$ shows that Compton heating alone is insufficient to stabilize the flow against cooling
