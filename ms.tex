...
accretion rate on large scales.
...
heats the electrons. For $\eddrt\gsim 0.1$, $\epsilon_{\rm rad}$
should saturate to 0.1, hence the fourth line. The third line is a
power law which connects $\epsilon_{\rm rad} (\eddrt=0.01) = 0.026$ to
$\epsilon_{\rm rad} (\eddrt=0.1) = 0.1$. The tildes are put in as a
reminder that the efficiency will depend on the small scale $\Mdot$
($\tilde{\dot{M}}$) which will in general be different from the large
scale $\Mdot$.
...
equation~\eqref{eq:efficiency}. This means that the bolometric
...
we obtain {\bf AG note the change in the description of the
...
the mean $r_b-M_{\bullet}$ relationship \citep{LauerFaber+:2007a}. We
also note that other constraints (the observed jet power, the jet
shut-off (if linked with the accretion becoming sub-Eddington), and an
observed QPO in the x-ray light curve) favor a smaller black hole
($10^5-10^6 \Msun$) \citep{Tchekhovskoy+2014}.
