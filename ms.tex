...
\caption{\label{fig:eta} Parameter $\eta$ characterizing stellar mass
  ...
  \citet{MosterNaab+:2013a} (see appendix~\ref{app:windheat} for details).}
\end{figure}
...
\caption{\label{fig:rs_rIa} $\rIa/\rs$ vs. $\Mbh$. $\rIa$ is
  ...
  corresponding to each $\Mbh$--see the black line
...
This is shown Figure~\ref{fig:miller} shows our predicted ${\rm
  L_{X}/L_{\rm edd}=\epsilon_X \Mdot c^2}$ for core (blue) and cusp
  (green) galaxies superimposed with x-ray measurements (red stars),
  upper limits (red triangles) from \citet{Miller+15}. $\epsilon_X$ is
  an efficiency factor and $\Mdot$ is calculated using
  Equation~\eqref{eq:mdot_analytic}, with parameters consistent with
  the average star formation history for each $\Mbh$ (see the black
  lines in Figures~\ref{fig:vwSources} and~\ref{fig:eta}). Our
  predicted trend in  $L_{X}/L_{\rm edd}$ is steeper than the 
  trend found in \citet{Miller+14} (black line). Note that we try two 
  different prescriptions for $\epsilon_X$: $\epsilon_X=10^{-4}$ in
  the left-hand panel and $\epsilon_X=10^{-1} \Mdot/0.01 \MdotEdd$
  right-hand panel. In either case our predicted trend in $
  L_{X}/L_{\rm edd}$ is opposite the observationally inferred
  one. {\bf AG-Need a bit more discussion here.} We note that in the
  right hand panel the observed galaxies are below the upper limits
  from Ia supernovae and thermal instability and so may be accreting
  in a thermally stable state {\bf AG-but it does not mean that they
    actually are}. 
