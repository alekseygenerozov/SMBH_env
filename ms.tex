...
\begin{abstract}
We calculate steady-state, one-dimensional hydrodynamic profiles of hot gas in the nuclei of quiescent galaxies for a range of black hole mass $M_{\bullet}$, parameterized gas heating rate, and observationally-motivated stellar density profiles.  Mass is supplied to the circumnuclear medium (CNM) by stellar winds, while gas is heated primarily by stellar winds, supernovae, and black hole feedback.  Analytic estimates are derived for quantities including the stagnation radius (where the radial velocity of the gas passes through zero) and the black hole mass accretion rate, $\dot{M}$, as a function of $M_{\bullet}$ and heating efficiency, the latter of which is related to the star-formation history of the stellar population.  We assess the conditions under which radiative instabilities will develop absent black hole feedback, based on the heating supplied by the average star formation history for galaxies corresponding to a given black hole mass.  Results for the average accretion rate, as well as the maximum thermally-stable and -unstable accretion rates, are compared to the nuclear X-ray luminosities of a sample of quiescent black holes.  We determine that the measured high accretion rates and short growth times of low mass black holes in the local universe must be the result of an external gas supply, rather than secular processes resulting from stellar mass loss within the galactic nucleus.  We briefly discuss implications of our results for the use of an assumed $\dot{M}-M_{\bullet}$ relationship to constrain the occupation fraction of SMBHs in low mass galaxies and the diversity of CNM environments encountered by the jetted outflows from tidal disruption events.
\end{abstract}
...
\end{align}
where for the numerical estimate we adopt a typical value of $n = 1$.  This expression is similar to that obtained by \citet{Volonteri+11} on more heuristic grounds (their eq.~6).  
...
\label{eq:rhors2}
\end{align}
that results by combining equations (\ref{eq:Tanalytic}), \eqref{eq:mdot_analytic}, and~\eqref{eq:mdot_gas}.  \citet{McCourt+12} have argued that cooling instability develops in a hydrostatic atmosphere if $t_{\rm cool} \ll t_{\rm ff}$.
...
\begin{figure}
  ...
  \caption{\label{fig:profiles}Radial profiles of the CNM density
    ...
    the stagnation radius for each solution {\bf BDM: added colored labels for the $v_w$ value, e.g. a blue 1200 km/s directly to the top figure} .
 }
\end{figure}
...
stagnation radius.  $^{(e)}$ Is solution thermally unstable?
...
Energy and mass input to the CNM by stellar winds is the sum of contributions from main sequence and post-main sequence stars.  At early times following star formation, energy input is dominated by core collapse supernovae and by the fast outflows of Wolf-Rayet stars (e.g., \citealt{VossDiehl+:2009a}).  At later times energy input is dominated by main sequence winds (e.g., \citealt{NaimanSoares-Furtado+:2013a}).  Mass input is also dominated by massive stars for very young stellar populations, but for most stellar ages the slow AGB winds of evolved low mass stars dominates the mass budget.  

In Appendix \ref{app:windheat} we calculate the wind heating rate from stellar winds, $v_{\rm w}^{\star}$, and the mass loss parameter, $\eta$ (eq.~[\ref{eq:q}]), as a function of age, $\tau_{\star}$, of a stellar population which is formed impulsively (Figs.~\ref{fig:vwimp}, \ref{fig:etaImp}).  At the earliest times ($\tau_{\star} \lesssim 10^{7}$ yr), the wind heating rate exceeds 1000 km s$^{-1}$, while much later ($\tau_{\star} \sim t_{\rm h}$) stellar wind heating dominated by main sequence winds is much lower, $v^{\star}_{\rm w} \sim 50-100 $ km s$^{-1}$.  We show in $\S\ref{sec:combined}$ that for the case of quasi-continueous star formation representative of the average star formation history of loss mass galaxies, the stellar wind heating rate produced by stellar winds can also be significant, $v_{\rm w}^{\star} \gtrsim 1000$ km s$^{-1}$.  Stellar wind heating thus contributes a potentially important source of both energy and mass for the CNM.
...
case $v_{w}^{\rm MSP} \lesssim 30$ km s$^{-1}$.  For higher spin-down
...
under the optimistc assumption $\epsilon_{\rm msp} = 1$.  However, the
stellar binaries giving rise to MSPs may be disassociated by stellar interactions in the dense nuclear cluster, reducing the numbers of MSPs as compared to the field population estimate above.  MSPs are thus unlikely to be an important source of heating.
...
emission (e.g.~\citealt{Narayan&Yi95}).  Note that, unlike stellar wind heating, Compton heating depends on radius as $v_{w}^{\rm C}(r) \propto (n^{2}\xi/\rho_{\star})^{1/2} \propto r^{(\Gamma-n-1)/2}$, i.e. $\propto r^{-0.6}$ and $\propto r^{-0.7}$ for core ($\Gamma = 0.8$; n = 1) and cusp ($\Gamma = 0.1$; n = 0.5) galaxies, respectively.

If heating is provided by Compton heating alone, i.e. $\tilde{v}_{w} = v_{w}^{C}$, then equation
...
Kinetic feedback results from outflows of energy or momentum in the form of a disk wind or jet from close to the black hole, which deposits its energy over much larger radial scales (e.g.~\citealt{McNamara&Nulsen07}; \citealt{Novak+11}; \citealt{Gaspari+12}).  

Assume that the outflow power is proportional to the SMBH accretion rate, $L_{\rm j} =
\epsilon_{\rm j} \dot{M}c^{2}$, where $\epsilon_{\rm j} < 1$ is an efficiency factor, and that this energy is deposited as heat uniformly interior to a radius $r_{\rm heat}$ and volume $V_{\rm heat} \propto r_{\rm
heat}^{3}$.  The resulting volumetric heating rate $\dot{e} = \epsilon_{\rm
j}\dot{M}c^{2}/V_{\rm heat}$ near the stagnation radius produces an effective wind velocity given by
...
M_{\bullet}(r/r_{\rm soi})^{2-\Gamma}$ (eq.~[\ref{eq:rhostar}]).  If most of the outflow energy is released close to the stagnation ardius, $r_{\rm heat} \sim r_{\rm s}$, then even a small heating efficiency $\epsilon_{\rm j} \gtrsim
10^{-5}$ is sufficient for $v_{w}^{\bullet}$ to exceed other sources
of non-accretion powered heating.  However, if the disk outflow instead distributes energy over much larger scales comparable to the size of the galaxy, i.e. $r_{\rm heat} \gtrsim $ 10 kpc $\sim 10^{4}r_{\rm s}$, then even for an efficient jet, kinetic feedback is unimportant.
...
burst of star formation Figure~\ref{fig:etaImp}.  The Ia heating rate
...
stellar populations in galaxies with $\Mbh\lsim 10^{7.5} \Msun$
results in $v_{w}^{\star}$ being dominant.  For $\Mbh\gsim 10^{7.5}
...
\caption{\label{fig:vwSourcesImp} Blah Blah}
\end{figure}
...
the black lines in Figures~\ref{fig:vwSource} and~\ref{fig:eta}). We
...
\caption{\label{fig:miller} $\epsilon_X \Mdot c^2/L_{edd}$ predicted
  from our models compared to $L_X/L_{Edd}$ values (red stars) and
  ...
  blue curves represent our predictions for core ($\Gamma=0.1$, blue)
  and cusp ($\Gamma=0.8$) galaxies respectively. They are calculated
  from equation~\eqref{eq:mdot_analytic}. The wind heating
  parameter, $v_w$ and mass return parameter, $\eta$, calculated for
  the average star formation histories in \citet{MosterNaab+:2013a}
  (the black lines in Figures~\ref{fig:vwSources}
  and~\ref{fig:eta}). Note that the star formation histories in
  \citet{MosterNaab+:2013a} are in terms of halo mass and for each
  $\Mbh$ we assign the halo mass, whose star formation history would
  produce a bulge consistent with the $\Mbh-M_{\rm bulge}$
  relationship from \citet{McConnellMa:2013a}.  We also show upper
  limits $\epsilon_X \Mdot c^2/L_{edd}$ from thermal stability
  (purple) and Ia supernovae (orange).}
\end{figure*}
