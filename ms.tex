...
heats the electrons. $\epsilon_{\rm rad}$ should saturate to 0.1 at
larger values of $\eddrt>0.01$, but such a black hole would certainly
be active and thermally unstable (and thus outside the scope of our
models). We note they $\eddrt<0.01$ for all of our models.
...
where we have used the facts that $\dot{M} = \eta M_{\star}|_{r_{\rm
    s}}/t_{\rm h}$ and, for radii inside the Nuker break radius,
...
heating efficiency $\epsilon_{\rm j} \gtrsim 10^{-5}$ is sufficient
...
kinetic feedback is unimportant, even for a powerful jet with
...
\caption{\label{fig:vwSources} {\it Top Panel:} Sources contributing
  ...
  $\zeta=\sqrt{1+\frac{v_w^2}{\sigma_0^2}}$ to the critical heating
  rate $\zeta_c$ for which no outflow is possible (see
  eq.~\eqref{eq:zetacrit}). In evaluating the latter quantity we use
  scaling relations for $r_b$, $\rinf$, and $\Gamma$ based on
  \citet{LauerFaber+:2007a}. {\bf AG added the zeta/zetac line}.}
...
$v_{\rm w}^{\rm Ia}$ depends on the total heating rate
(eq.~[\ref{eq:vtot}]), requiring us to simultaneously solve a series
of implicit equations to determine each heating rate.  Heating from
MSPs is found to be negligibly small for fiducial parameters and hence
is not shown, while kinetic feedback is neglected given its uncertain
efficiency ($\S\ref{sec:kinetic}$).
