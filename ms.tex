...
\begin{abstract}
  ...
  M_{\bullet}^{\alpha}$ with $\alpha \approx 0.5, 1$ for cups and core
  nuclei, respectively.
...
nominal Bondi radius, the difference in the accretion rate can be
significant.  Furthermore, in some cases we find that no stagnation
...
accretion rate than would be predicted by simple Bondi accretion.  The
main goal of our analysis is to provide a framework for interpreting
the properties of quiescent galactic nuclei given the relatively well
understood physics of stellar winds and supernova heating.
...
% The second source term in
% equation (\ref{eq:dsdt}) accounts for heating transportation by
% electron conductivity, where the conductivity $\kappa = \kappa_0
% T^{5/2}$ is assumed to equal the classical ``Spitzer" value
% (\citealt{Spitzer62}); although even a weak magnetic field could in
% principle strongly suppress the conductivity perpendicular to the
% field, the radially-decreasing temperature profiles of interest are
% susceptible to the magneto-thermal instability (\citealt{Balbus01}),
% the non-linear evolution of which is believed to result in a
% radially-directed field structure (\citealt{Parrish&Stone07}).
...
shallower than the $-3/2$ for Bondi accretion. However, the power law
behavior does not extend through all radii. NGC4365 which has a
prominent break in the gas density profile, near the stagnation
radius. This is also roughly the location of the break in the stellar
light profile ($\rb$). The temperature profile is relatively flat at
large radii, but increases as $\propto 1/r^{k}$ interior to the sphere
of influence, where $k$ is smaller than 1 (which would be expected for
virialized gas).  The inwardly directed velocity increases towards the
hole approaching the expected free-fall rate $v \propto v_{\rm
  ff}\propto r^{-1/2}$ interior to the sonic point, and then increases
outwards towards large radii.
...
s$^{-1}$.  
%In Appendix C we present a toy model for how $v_{\rm w,0}$
%varies as a function of stellar age.
%AG-Appendix C is not written! As this draft will be circulated I
%don't want references to things which do not exist!
...
$\rs>\rIa$. In this regime, we add the $\vwO$ values from SNe Ias and
from main sequence winds in quadrature. $\vwO$ from SNe Ia's depends
on the rate of mass loss ($\eta$), which is in turn a function of
$t_{\star}$. This dependence is why the effective $\vwO$ increases
with $\tage$ for large $\Mbh$.
...
upper limits are shown as the blue and black solid lines
respectively. The points correspond to the $\dot{M} c^2$ for our
solutions (scaled by $5\times 10^{-7}$).  We must choose a $\vwO$ for
each galaxy.  We select $\vwO$=200 km/s if $\rs<\rIa$ for this $\vwO$,
$\vwO$=500 km/s if $\rs>\rIa$ for this $\vwO$, or exclude it. If
...
a wind heating parameter $\vwO=500$ km s$^{-1}$ (close to, but
somewhat higher than that expected from SNe Ia) gives rough agreement
...
\caption{\label{fig:bh_xray} Accretion power plotted as a function of
  stellar mass for our solutions (points) and the trend of nuclear
  x-ray luminosity with stellar mass (black line) and the detection
  upper limit (blue line) from Fig. 1 of
  \citealt{MillerGallo+:2014a}. In the top panel we take the accretion
  power to be $\epsilon_1 \Mdot c^2$, with $\epsilon_1=5\times
  10^{-7}$ chosen to make our accretion powers comparable to the
  observed x-ray luminosities. In the bottom panel, we take the
  accretion power to be quadratic in $\Mdot$--$\epsilon_2 \eddr \Mdot
  c^2$ with $\epsilon2$=$2\times10^{-4}$. For a given galaxy, we take
  either $\vwO=200$ km/s (green points) or $\vwO=500$ km/s (orange
  points). We choose the solution for which the relative positions of
  $\rs$ and $\rIa$ are consistent with the assumed level of heating}
...
\label{sec:conduction}{
  ...
  \item We compare our solutions for density and temperature to those
    inferred from Chandra x-ray observations by \citealt{Allen+:2006a}
    and \citealt{RussellMcNamara+:2013a} for a handful of galaxies in
