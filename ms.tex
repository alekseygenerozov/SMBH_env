...
\caption{\label{fig:bh_growth} {\it Top panel:} Gas inflow rate $\dot{M}/\dot{M}_{\rm edd}$ as a
  ...
  radius) {\it Bottom panel:} Growth times $\Mbh/\dot{M}_{\bullet}$ in units of
  the Hubble time $\th$ for each of the accretion rates shown in the
  top panel, where $\dot{M}_{\bullet} = 0.1\dot{M}$ to account for the fraction of the inflowing matter lost to outflows from the accretion disk.}
\end{figure}
...
\caption{\label{fig:miller} Average nuclear X-ray luminosity, $\langle
  ...
  panel assumes $\epsilon_X= \alpha \epsilon_{\rm bol}\epsilon_{\rm rad}$, where $\epsilon_{\rm rad}$ is the radiative efficiency calculated by \citet{Sharma+2007} (eq.~[\ref{eq:efficiency}]) and $\alpha = 0.1$ is the fraction of the mass inflowing on large scales that reaches the SMBH.  Shown
