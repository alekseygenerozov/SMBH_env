...
accretion rates $10^{-4} \lsim \eddr \lsim 10^{-2}$, the radiative
efficiency is $\epsilon_{\rm rad}\sim 2.6\%$\footnote{for $\eddr\lsim
  10^{-4} \epsilon_{\rm rad}\sim \eddr^{0.9}$. For $\eddr\gg 0.01$
  $\epsilon_{\rm rad}\sim 0.1$...}. Additionally, a large
fraction of the $\dot{M}$ on large scales will be driven out in
outflows as the gas has some non-zero angular momentum and a
centrifugal barrier will stop it from accreting \citet{Li+2013}. In
the limit weak cooling appropriate for low accretion rates the
accretion rate on small scales will be of order $\alpha \Mdot$. Here
we adopt $\alpha=0.1$ so that our overall efficiency is 0.26\%
...
\caption{\label{fig:miller} Average nuclear X-ray luminosity, $\langle
  ...
  (\alpha/0.1) \epsilon_{rad} (\eddr) $, where $\epsilon_{\rm
    rad}\simeq 0.026$ for $\lsim 10^{-4} \eddr\lsim 10^{-2} $, as
  motivated by shearing box simulations of collisionless plasma (see
  \citet{Sharma+2007}).  Shown for comparison are the X-ray
