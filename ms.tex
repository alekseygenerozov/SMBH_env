...
\label{eq:qdot_rad}
\ee
where $n = \rho/\mu m_p$ is the gas number density and $\Lambda(T)$ is the cooling function.  We neglect radiative cooling in our baseline calculations, despite the fact that this is not justified when the wind heating $\vwO$ is low and/or stellar mass loss rate $\eta$ is high.  However, once radiative cooling becomes important compared to other sources of heating and cooling, its presence leads to thermal instability (e.g.~\citealt{McCourt+12}) that our 1D
time-independent model cannot account for anyways.  A main goal of this work is to use analytic and numerical solutions neglecting radiation to ascertain over what range of physical conditions cooling instabilities are most likely to develop ($\S\ref{sec:cooling}$).
...
\label{eq:bondi}
\end{align}
where $r_{\rm B} \equiv GM/c_{\rm s,ad}^{2}$ is the Bondi radius, $c_{\rm s,ad} = \sqrt{\gamma kT/\mu m_p}$ is the adiabatic sound speed, $v_{\rm ff}|_{r_{\rm B}} = r_{\rm B}/t_{\rm ff}|_{r_{\rm B}} = (GM_{\bullet}/r_{\rm B})^{1/2}$ and $\lambda$ is a parameter of order unity.  One observes a similarity between equation (\ref{eq:mdot_gas}) and the Bondi formula (eq.~[\ref{eq:bondi}]) if $R_{\rm B}$ is replaced by $\rs$.  When the stagnation radius resides interior to the sphere of influence we indeed have (eq.~[\ref{eq:stag_simple}])
\begin{align}
...
Our analytic results neglect the effects of heat conduction, an assumption we now check.  The ratio of the magnitude of the conductive heating rate to the nominal external (``wind") heating rate at the stagnation radius is given by
\begin{align}
  \left.\frac{\nabla\cdot(\kappa \nabla T)}{q v_{\rm
...
\hline
{$M_{\bullet,8}$} & {$v_{w}$} & {$\Gamma^{(a)}$} & {$r_b^{(b)}$} &
{$\left(\frac{\dot{M}}{\dot{M}_{\rm edd}}\frac{0.04}{\eta}\right)_{r_{s}}$} & {$\left(\frac{t_{\rm cool}}{t_{\rm ff}}\frac{0.04}{\eta}\right)_{r_s}$$^{(c)}$} & TI?$^{(d)}$ & {comments}\\
\hline
0.01 & 300 & 0.8 & $>$XXX & $ 3.2 \times 10^{ -4 }$ & 4.7 & N & rb in fact
unresolved \\
0.01 & 600 & 0.8 & $>$XXX & $ 5.5 \times 10^{ -5 }$ & $ 3.2 \times
10^{ 2 }$ & N & \\
0.01 & 1200 & 0.8 & $>$XXX & $ 9.4 \times 10^{ -6 }$ & $ 1.5 \times
10^{ 4 }$ &  N &\\
0.1 & 300 & 0.8 & 100 & $ 2.5 \times 10^{ -3 }$ & 0.29 & N &\\
0.1 & 600 & 0.8 & 100 & $ 1.8 \times 10^{ -4 }$ & 97 &N  & \\
0.1 & 1200 & 0.8 & $>$ & $ 3.1 \times 10^{ -5 }$ & $ 4.8 \times
10^{ 3 }$ & N & \\
\hline
1 & 300 & 0.8 & 100 & $ 1.6 \times 10^{ -2 }$ & $ 5.6 \times 10^{
  -2 }$ & Y  & \\
1 & 600 & 0.8 & 100 & $ 7.3 \times 10^{ -4 }$ & 21 & N & \\
1 & 1200 & 0.8 & 100 & $ 9.9 \times 10^{ -5 }$ & $ 1.5 \times 10^{
  3 }$ & N &  \\
1 & 300 & 0.8 & 50 & $ 8.2 \times 10^{ -3 }$ & 0.19 & N & \\
1$\dagger$ & 300 & 0.8 & 50 & $ 9.1 \times 10^{ -4 }$ & 5.6 & N & \\
1 & 300 & 0.8 & 200 & $ 3.6 \times 10^{ -2 }$ & $ 1.7 \times 10^{
  -2 }$ &Y & TI/convergence \\
%1 & 300.0 & 0.8 & 300.0 & $ 5.2 \times 10^{ -2 }$ & $ 9.8 \times 10^{
%  -3 }$ &  TI/convergence\\
1 & 300 & 0.8 & 400 &  &  & Y & TI/run unsuccessful\\
0.1 & 300 & 0.8 & 50 & $ 2.0 \times 10^{ -3 }$ & 0.46 & N &\\
0.1 & 300 & 0.8 & 200 & $ 5.3 \times 10^{ -3 }$ & 0.12 & Y & TI/convergence\\
%0.1 & 300.0 & 0.8 & 300.0 & $ 7.2 \times 10^{ -3 }$ & $ 6.7 \times
%10^{ -2 }$   & TI/convergence  \\
0.1 & 300 & 0.8 & 400 & $ 8.9 \times 10^{ -3 }$ & $ 4.7 \times
10^{ -2 }$ & Y &TI/convergence \\
0.01 & 1200 & 0.1 & U & $ 1.5 \times 10^{ -6 }$ & $ 4.1 \times
10^{ 4 }$ & N & \\
0.1 & 1200 & 0.1 & U & $ 9.5 \times 10^{ -6 }$ & $ 5.7 \times 10^{
  3 }$ & N & tried run in which rb=100 pc was resolved what would happen if rb=400 pc?\\
0.01 & 1200 & 0.1 & 100 & $ 1.5 \times 10^{ -6 }$ & $ 4.1 \times 10^{ 4 }$ &N  &\\
0.1 & 1200& 0.1 & 100 & $ 9.5 \times 10^{ -6 }$ & $ 5.7 \times 10^{ 3 }$ &N&  \\
1 & 1200 & 0.1 & 100 & $ 6.0 \times 10^{ -5 }$ & $ 8.6 \times 10^{ 2 }$ & N& \\
1 & 1200 & 0.1 & 50 & $ 4.9 \times 10^{ -5 }$ & $ 1.2 \times 10^{ 3 }$ &N  &\\
1 & 1200 & 0.1 & 200 & $ 1.0 \times 10^{ -4 }$ & $ 3.4 \times 10^{ 2 }$ &N&  \\
%1 & 1200.0 & 0.1 & 300.0 & $ 1.9 \times 10^{ -4 }$ & $ 1.3 \times 10^{ 2 }$ &  convergence\\
1 & 1200 & 0.1 & 400 & $ 1.8 \times 10^{ -2 }$ & 0.37 & Y &TI/convergence\\
\hline
\label{table:models}  
\end{tabular}
$^{(a)}$ Inner power-law index of stellar Nuker profile.  $^{(b)}$ Break radius of stellar Nuker profile.  $^{(c)}$Ratio of cooling timescale to free-fall timescale at stagnation radius.  $^{(d)}$ Is solution thermally unstable?  $\dagger$Calculated including radiative cooling, assuming $\eta = 0.04$.  {\bf BDM: (1) remove comments column, (2) update numbers assuming eta = 0.04, (3) make sure all combinations of vw and mbh are included}
\end{minipage}
\end{table*}
...
Figure~\ref{fig:stag} shows our results for the stagnation radius $r_{\rm s}$, expressed in ratio to sphere of influence radius $r_{\rm soi}$, with different colors showing different values of $v_{w}$.  Cusp and core galaxies are marked with square and triangles, respectively.  Shown for comparison are our analytic results (eq.~[\ref{eq:rs2main}]) with solid and dashed lines for cusp and core galaxies, respectively.  Our analytic estimates provide reasonably accurate fits to the numerical results for $v_{\rm w} \gtrsim \sigma$, but for low $v_{\rm w}$ they break down as the stagnation radius diverges to large radii.  For $v_{w} \gg \sigma$ the value $\rs/\rsoi$ does not depend sensitively on break radius $r_{b}$, but the latter has a greater influence when $v_{w} \lesssim \sigma$.  The three green squares located at the same value of $\vw =300 {\rm km/s}$, close to where the solid analytic line diverges, correspond to models with the same black hole masses $\Mbh = 10^7 \Msun=$ but different values of the break radius: $r_{b} = $ 50, 100, and 400 pc (from top to bottom).  
...
\begin{figure}
  ...
  \caption{\label{fig:stag} \emph{Top panel:} Stagnation radius
    ...
    analytic prediction from equation (\ref{eq:rs2main}), with solid and dashed curves calculated for parameters $\{\Gamma=0.8, n=1\}$ and $\{\Gamma=0.1,n=0.5\}$, respectively.  {\bf BDM: is the x axis here $v_{w}$ or $\vw$ !!?}}
\end{figure}
