...
\begin{tabular}{lccccccccccccc}
  ...
  {$\left(\frac{\dot{M}}{\dot{M}_{\rm edd}}\frac{0.04}{\eta}\right)_{r_{s}}^{(c)}$} & {$\left(\frac{t_{\rm cool}}{t_{\rm ff}}\frac{0.04}{\eta}\right)_{r_s}^{(d)}$}  \\
  \hline
  $    10^{ 6 }$ & 300 & 0.8 & 100 & $ 1.3 \times 10^{ -5 }$ & $ 1.1 \times 10^{ 2 }$ \\
  $    10^{ 6 }$ & 600 & 0.8 & 100 & $ 2.2 \times 10^{ -6 }$ & $ 8.1 \times 10^{ 3 }$ \\
  $    10^{ 6 }$ & 1200 & 0.8 & 100 & $ 3.8 \times 10^{ -7 }$ & $ 3.7 \times 10^{ 5 }$ \\
  $    10^{ 7 }$ & 300 & 0.8 & 100 & $ 1.0 \times 10^{ -4 }$ & 6.5 \\
  $    10^{ 7 }$ & 600 & 0.8 & 100 & $ 7.2 \times 10^{ -6 }$ & $ 2.4 \times 10^{ 3 }$ \\
  $    10^{ 7 }$ & 1200 & 0.8 & 100 & $ 1.3 \times 10^{ -6 }$ & $ 1.2 \times 10^{ 5 }$ \\
  $    10^{ 8 }$ & 300 & 0.8 & 100 & $ 6.6 \times 10^{ -4 }$ & $\mathbf{1.4}$ \\
  $    10^{ 8 }$ & 600 & 0.8 & 100 & $ 2.9 \times 10^{ -5 }$ & $ 5.3 \times 10^{ 2 }$ \\
  $    10^{ 8 }$ & 1200 & 0.8 & 100 & $ 4.0 \times 10^{ -6 }$ & $ 3.8 \times 10^{ 4 }$ \\
  $    10^{ 7 }$ & 300 & 0.8 & 25 & $ 7.8 \times 10^{ -5 }$ & 17 \\
  $    10^{ 7 }$ & 300 & 0.8 & 50 & $ 7.9 \times 10^{ -5 }$ & 12 \\
  $    10^{ 7 }$ & 300 & 0.8 & 200 & $ 1.2 \times 10^{ -4 }$ & $\mathbf{4.7}$ \\
  $    10^{ 7 }$ & 300 & 0.8 & 400 & $ 3.6 \times 10^{ -4 }$ & $\mathbf{1.2}$ \\
  $    10^{ 8 }$ & 300 & 0.8 & 25 & $ 1.9 \times 10^{ -4 }$ & 12 \\
  $    10^{ 8 }$ & 300 & 0.8 & 50 & $ 3.3 \times 10^{ -4 }$ &
  $\mathbf{4.6}$ \\
  $    10^{ 8 \dagger}$ & 300 & 0.8 & 50 & $ 9.1 \times 10^{ -4 }$ &
  $\mathbf{5.6}$ \\
  $    10^{ 8 }$ & 300 & 0.8 & 200 & $ 1.4 \times 10^{ -3 }$ &
  $\mathbf{0.43}$ \\
  \hline
  $    10^{ 6 }$ & 1200 & 0.1 & 100 & $ 5.9 \times 10^{ -8 }$ & $ 1.0 \times 10^{ 6 }$ \\
  $    10^{ 7 }$ & 1200 & 0.1 & 100 & $ 5.1 \times 10^{ -7 }$ & $ 9.4 \times 10^{ 4 }$ \\
  $    10^{ 8 }$ & 1200 & 0.1 & 100 & $ 2.4 \times 10^{ -6 }$ & $ 2.1 \times 10^{ 4 }$ \\
  $    10^{ 8 }$ & 1200 & 0.1 & 50 & $ 2.0 \times 10^{ -6 }$ & $ 3.1 \times 10^{ 4 }$ \\
  $    10^{ 8 }$ & 1200 & 0.1 & 200 & $ 4.2 \times 10^{ -6 }$ & $ 8.4 \times 10^{ 3 }$ \\
  $    10^{ 8 }$ & 1200 & 0.1 & 400 & $ 1.2 \times 10^{ -3 }$ & $\mathbf{4.6}$ \\
  $    10^{ 8 }$ & 600 & 0.1 & 100 & $ 8.0 \times 10^{ -4 }$ & $\mathbf{1.6}$ \\
  $    10^{ 6 }$ & 600 & 0.1 & 25 & $ 1.6 \times 10^{ -6 }$ & $ 1.6 \times 10^{ 3 }$ \\
  $    10^{ 6 }$ & 600 & 0.1 & 50 & $ 1.7 \times 10^{ -5 }$ & $\mathbf{33}$ \\
  $    10^{ 7 }$ & 600 & 0.1 & 25 & $ 8.5 \times 10^{ -6 }$ & $ 4.6 \times 10^{ 2 }$ \\
  $    10^{ 7 }$ & 600 & 0.1 & 50 & $ 4.5 \times 10^{ -5 }$ & $\mathbf{21}$ \\
  $    10^{ 8 }$ & 600 & 0.1 & 25 & $ 2.3 \times 10^{ -5 }$ & $ 2.8 \times 10^{ 2 }$ \\
  $    10^{ 8 }$ & 600 & 0.1 & 50 & $ 6.0 \times 10^{ -5 }$ & 56 \\
  \hline
...
text.
 {\bf  BDM: (3) make sure all combinations of vw and mbh
  ...
  solution may not be representative due to the small break radius.}
\end{tablenotes}
\end{minipage}
\end{threeparttable}
...
Figure~\ref{fig:stag} shows our results for the stagnation radius $r_{\rm s}$, expressed in ratio to sphere of influence radius $r_{\rm soi}$, with different colors showing different values of $v_{w}$.  Cusp and core galaxies are marked with square and triangles, respectively.  Shown for comparison are our analytic results (eq.~[\ref{eq:rs2main}]) with solid and dashed lines for cusp and core galaxies, respectively, calculated assuming $n = 1$ and $n= 0.5$, respectively.  Our analytic estimates are found to provide accurate fits to the numerical results for $v_{\rm w} \gtrsim \sigma$, but the assumptions leading to these estimates become invalid for low $v_{\rm w}$ as the stagnation radius diverges to large radii.  For $v_{w} \gg \sigma$ the ratio $\rs/\rsoi$ does not depend sensitively on break radius $r_{b}$, but the latter has a greater influence when $v_{w} \lesssim \sigma$.  The three green squares which are vertically aligned at the same value of $v_{w}/\sigma$ (close to where the analytic estimate diverges) correspond to models with the same black hole mass ($\Mbh = 10^7 \Msun$) and heating rate ($v_w = 300$ km s$^{-1}$) but different values of the break radius: $r_{b} = $ 50, 100, and 400 pc (from top to bottom).  As $r_{\rm b} \rightarrow r_{s}$, the effects of the break radius is to the push the stagnation radius inwards, decreasing the SMBH accretion rate.    
 ...
 Figure~\ref{fig:mdot_mass} shows the SMBH accretion rate for each of our numerical solutions in Eddington units for different values of $\vwO =$ 300, 600 and 1200 km s$^{-1}$, and for both core ($\Gamma$=0.1) and cusp galaxies ($\Gamma$=0.8).  {\bf BDM: say something smart}
...
\begin{eqnarray}
\frac{\dot{M}_{\rm Ia}}{\dot{M}_{\rm edd}} &\approx& \frac{\eta M_{\bullet}}{\dot{M}_{\rm edd} t_{\rm h}}\left(\frac{R_{\rm Ia}}{R_{\rm soi}g}\right)^{2-\Gamma} \approx \nonumber \\
 && \begin{cases}
...
\caption{\label{fig:vwSources} {it Top Panel:} Sources contributing   to the heating rate of the CNM, $\vwO$, including stellar wind heating ({\it orange}), Ia supernovae ({\it green}), millisecond pulsars ({\it blue}), and compton heating ({\it pink}).  Each heating source varies with black hole mass $\Mbh$ due to the different
  ...
  heating rate is shown in black. {\it Bottom Panel:} The
...
The X-ray luminosities of galactic nuclei provide a powerful diagnostic of the SMBH accretion rates and how they vary across different galaxy properties and SMBH masses (e.g.,  \citealt{Ho08}  and references therein).  Figure \ref{fig:vwSources} shows that the heating associated with the average star formation history of a galaxy is approximately constant with SMBH mass, except for the largest black holes with $M_{\bullet} \gtrsim 5\times 10^{8}M_{\odot}$.  For a fixed wind heating parameter, the Eddington ratio $\dot{M}/\dot{M}_{\rm edd}$ increases $\propto M_{\bullet}^{0.5(0.8)}$ for core(cusp) galaxies, respectively (Fig.~\ref{fig:mdot_mass}; eq.~[\ref{eq:eddr_analytic}]).  This dependence, shown directly in Figure XXX, appears to contradict the observed behavior of the nuclear X-ray luminosities of quiescent galaxies, which (though showing several orders of magnitude scatter) instead on average obey $\langle L_X/L_{\rm edd}\rangle \sim \Mbh^\alpha$, where $\alpha \in [-0.4,-0.2]$ \citep{Zhang+09, Pellegrini10, Gallo+10, Miller+15}.   

Figure~\ref{fig:miller} shows our predictions for $\langle L_{X} \rangle =\epsilon_X \epsilon_{\rm bol} \dot{M} c^2$ in Eddington units as a function of black hole mass for core (blue) and cusp (green) galaxies, where $\epsilon_{\rm bol} = L_{X}/L_{\rm bol}= 0.1$ is the assumed bolometric correction appropriate for low luminosity AGN (\citealt{Ho08}).  The left panel is calculated assuming a constant low value for the radiative efficiency $\epsilon_X = 10^{-4}$, typical of those estimated for low luminosity AGN (e.g., \citealt{Ho:2009a}) and SgrA* (e.g.,
\citealt{Quataert+99}).  The luminosity shown in the right
panel instead assumes an efficiency of $\epsilon_X= \dot{M}/\dot{M}_{\rm edd}$ that depends on $\dot{M}$ in the way predicted by models for radiatively inefficient accretion flows (\citealt{Narayan&Yi95}).  Plotted next to the theoretical curves are the X-ray measurements (stars) and upper limits
(triangles) from \citet{Miller+15} (cf.~\citealt{Gallo+10}), along with the resulting best power-law
fit (red line) and the X-ray detection threshold (gray line).  Purple, orange, and {\bf brown} lines show the maximum accretion rates for thermally stable accretion (eq.~[\ref{eq:Mdotmax}]), as set by SN Ia blow-out
(eq.~[\ref{eq:eddr_Ia}]), and as allowed by Compton heating feedback (eq.~[\ref{eq:MdotC}]), respectively.

To first order our predicted values of $\langle L_{X}/L_{\rm edd}\rangle$ for the average galactic star formation histories overlap with the data reasonably well.  Our prediction that $\langle L_{X}/L_{\rm edd}\rangle$ should increase with black hole mass appears to be in disagreement with observed decrease of $\langle L_{X}/L_{\rm edd}\rangle$ with $M_{\bullet}$ for both core and cusp galaxies.  However, better agreement is obtained by taking into account the fact that core(cusp) galaxie are more commonly associated with larger(smaller) black hole masses, such that when both samples are combined one predicts a flatter dependence of $\langle L_X/L_{\rm edd}\rangle$ of $M_{\bullet}$.   The large scatter observed in $L_{X}/L_{\rm edd}$ of typically 2-4 orders of magnitude can also be understood within a steady-accretion scenario due to the strong sensitivity of the accretion rate on the efficacy of gas heating, $\dot{M}/\dot{M}_{\rm edd} \propto v_{w}^{-3.8(-2.4)}$ for core(cusp) galaxies, respectively (Fig.~\ref{fig:mdot_mass}; eq.~[\ref{eq:eddr_analytic}]) and the equally sensitive dependence of the heating rate on stochastic intermittency in the star formation history (Fig.~NICK).  

In the case of an old stellar population which produces insufficient heating to support steady accretion, this large scatter can in principle also be understood by the wide range of gas masses experienced over the course of a cyclic episode between instabilities and periodic nuclear outbursts (e.g.~\citealt{Ciotti+10}).  The cyclic model also predicts that high $L_{X}$ nuclei should be more common in galaxies with younger stellar ages.  This appears in tension with the finding by \citet{Pellegrini10} that galaxies with younger stellar populations are observed to host less X-ray luminous nuclei, at least for stellar ages $<3$ Gyr.  This observation can also be understood in the stable accretion picture by the fact that young stars provide sufficient stellar wind heating to produce a thermally stable accretion state.  

However, the observed values of 
$\langle L_{X}/L_{\rm edd}\rangle$ do imply the accretion rates of
observed galaxies are below the maximum ones for thermally stable
accretion if $\epsilon_X \lesssim 10^{-4}$.  This suggests that most
nearby galaxies are consistent with accreting in a thermally stable
steady state, even if our estimate of their predicted heating does not
match the observed trend with $M_{\bullet}$ perfectly.  On the other
hand if $\epsilon_{X} \sim 10^{-5}$ then the trend would be consistent
with the accretion rate set by Ia blow-out.  Also note that in the
case of $\dot{M}-$dependent radiative efficiency ($\epsilon_X =
\dot{M}/\dot{M}_{\rm edd}$), the predicted branches of $\langle
L_{X}/L_{\rm edd}\rangle$ decreases with decreasing $M_{\bullet}$,
calling into question attempts to extrapolate the $\langle L_{X}
\rangle - M_{\bullet}$ relationship to low $M_{\bullet}$ in order to
constrain the occupation fraction of black holes in the lowest mass
galaxies (\citealt{Miller+15}).

Past studies have found that core galaxies show a wide range of nuclear radio and X-ray luminosities, while cusp galaxies are confined below a threshold (e.g., \citealt{Bender+89, Pellegrini99, Capetti&Balmaverde05}).

Also clear from our results are that black holes accreting in AGN phase cannot be accreting in a steady 

\subsection{Steady Accretion versus Cyclic Outbursts}

GALEX observations have shown that a small fraction of early-type galaxies in the local universe have undergone recent ($< 1-2$ Gyr old) star formation episodes (\citealt{Donas+07}).  
...
This appears to be in contradiction with the general down sizing trend observed for SMBH accretion in the local universe (e.g.~\citealt{Heckman+04}; \citealt{Gallo+08})
...
\caption{\label{fig:bh_growth} {\it Bottom panel:} Growth times $\Mbh/\Mdot$ in units
  of the Hubble time $\th$ for core ($\Gamma=0.1$, solid black) and
  cusp galaxies ($\Gamma=0.8$, dash-dotted black). These are calculated from
  ...
  bound on growth time for a themally stable flow (yellow) from
