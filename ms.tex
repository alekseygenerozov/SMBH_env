...
Figure \ref{fig:vwSources} shows that the average heating rate is relatively constant with SMBH mass.  For fixed wind heating $\dot{M}/\dot{M}_{\rm edd}$ increases with SMBH mass $\propto M_{\bullet}^{0.5(0.8)}$ for core(cusp) galaxies, respectively (Fig.~\ref{fig:mdot_mass}; eq.~[\ref{eq:eddr_analytic}]).  This appears in contradiction with the nuclear X-ray luminosities of quiescent galaxies, which though showing broad scatter instead obey $L_X \sim \Mbh^\alpha$, where $\alpha\simeq 0.7-0.8$ \citep{Pellegrini10, Miller+15}.  Our model instead predicts that $\Mdot
...
This is shown Figure~\ref{fig:miller} shows our predicted ${\rm
  ...
  trend found in \citet{Miller+15} (black line). Note that we try two 
...
new gridded approach.  BDM: too bad!}
