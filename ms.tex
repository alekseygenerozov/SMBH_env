...
\begin{figure}
  ...
  \caption{\label{fig:stag} \emph{Top panel:} Stagnation radius
    ...
    curves calculated for parameters $\{\Gamma=0.8, \densSlope=1\}$ and
    $\{\Gamma=0.1,\densSlope=0.5\}$, respectively. }
\end{figure}
...
r^{(\Gamma-\densSlope-1)/2}$, i.e. $\propto r^{-0.6}$ and $\propto r^{-0.7}$
for core ($\Gamma = 0.8$; $\densSlope = 1$) and cusp ($\Gamma = 0.1$;
$\densSlope = 0.5$) galaxies, respectively.  The assumption of our model that the heating
