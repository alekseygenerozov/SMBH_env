...
\begin{abstract}
  ...
  winds, while energy is injected primarily by stellar winds, supernovae,
  and black hole feedback.  Analytic estimates are derived for the
  stagnation radius (where the radial velocity of the gas passes
  through zero) and the black hole accretion rate, $\dot{M}$, as a
  function of $M_{\bullet}$ and the gas heating efficiency, the latter
  being related to the star-formation history.  We assess the
  conditions under which radiative instabilities develop, both
  in the case of a single burst of star formation and for the average
  star formation history predicted by cosmological simulations.  By combining a sample
  of measured nuclear X-ray luminosities, $L_{X}$, of nearby quiescent
  galactic nuclei with our results for $\dot{M}(M_{\bullet})$ we show
  that most nuclei are consistent with accreting in a steady-state,
  thermally-stable manner.  However, thermally-stable accretion cannot
  explain the short average growth times of low mass black holes in
  the local universe, which must instead result from gas being fed in
  from large radii, due either to galaxy mergers or thermal
  instabilities acting on larger, galactic scales.  Our results have
  implications for attempts to constrain the occupation fraction of
  SMBHs in low mass galaxies using the mean $L_{\rm X}-M_{\bullet}$
  correlation, as well as the predicted diversity of the circumnuclear
  densities encountered by relativistic outflows from tidal disruption
  events.
\end{abstract}
...
analytic results, which are justified via the numerical solutions we present in
$\S\ref{sec:numerical}$.  We move from a general and parametrized treatment of heating to a physically motivated one in \S \ref{sec:heating}, where we consider a range of physical processes that can inject energy into the CNM.  In
...
\label{eq:sigmarel}
\ee 
is the stellar velocity dispersion and which accounts for the minimal amount of shock heating from stellar winds due to the random motion of stars in the SMBH potential, while $\sigma_0^{2} \approx 3 \sigma^2$ is assumed to be constant, corresponding to the observed
velocity dispersion outside of the SMBH sphere of influence
\citep{ChaeBernardi+:2014a}.  The second term, $v_{w}^{2}$, parameterizes additional sources of energy input, including faster winds from young
stars, millisecond pulsars, supernovae, AGN feedback, etc  ($\S\ref{sec:heating}$).  We assume
that $v_w$ is constant with radius, i.e. that the volumetric heating
rate is proportional to the local stellar density.  
%The value of $v_w$
...
critical threshold (Fig.~\ref{fig:zetaCrit}) \be \zeta\equiv \sqrt{1 + \frac{v_w}{\sigma_0}^2} <
\zeta_c \approx \left(\frac{\rb}{\rsoi}\right)^{0.5(1-\Gamma)}.
\label{eq:zetacrit}
\ee This result makes intuitive sense: gas is supplied to the nucleus
...
\sigma$) the stagnation radius lies well inside the black hole sphere of
...
\begin{figure}
  ...
  \caption{\label{fig:mdot_mass} Accretion rate $\dot{M}/\dot{M}_{\rm
    ...
    would be thermally unstable.  Solid and dot-dashed curves correspond
...
radius for our fiducial solutions shown in Figure \ref{fig:profiles}.  Radiative cooling is calculated using the cooling function from \citealt{Draine:2011a}.  For high heating rates of $v_{w} = 600$ and $1200$ km s$^{-1}$ cooling
...
\caption{\label{fig:vwSourcesImp} Different sources contributing to
  ...
  are the value of the stellar velocity dispersion, $\sigma_0$,
  approximated as $\sqrt{3}$ time the $\sigma$ from the $\Mbh-\sigma$
...
\item Heating from SMBH feedback, unlike stellar heating, depends on
  the SMBH accretion rate, which is itself a function of the heating
  rate.  Compton heating is generally unimportant compared to stellar
...
\item Low mass black holes grow in the low redshift universe over
  ...
  nucleaus, e.g. due to galaxy mergers or thermal instabilities on
  larger, galactic scales (e.g.~\citealt{Voit+15}).

%  \item Our calculation of $\Mdot$ agrees well with the one calculated from the Bondi formula. We find that they agree to
...
We conclude by drawing attention to the following limitations of our model.  First, by neglecting any large scale inflow of gas to the nucleus and assuming the only source of gas feeding the black hole is stellar wind mass loss from the local stellar population, our estimates represent a lower bound on the gas density of the CNM.  Our model therefore cannot account for most AGN activity.   

The predicted SMBH accretion rate is also strongly tied to the age and radial distribution of the stellar population within the sphere of influence.  Our model cannot make definitive predictions for the accretion mode of individual galaxies without knowledge of their specific SFH.

We adopt halo average star formation rates measured in cosmological simulations by \citet{MosterNaab+:2013a}, and neglect any spatial variation in the star formation rate for any given galaxy. Galaxies of mass $\gsim 7\times 10^{10} \Msun$, assemble their stars from the inside out \citep{PerezCidFernandes+:2013a}, such that for a given halo mass the galactic nucleus may be older than in our model and thus more prone to thermal instability.  On the other hand, \citealt{LauerFaber+:2005a} find that $\sim
      60\%$ of cusp galaxies and $\sim29\%$ of core galaxies have
      (generally unresolved) emission in excess of the inward
      extrapolation of the Nuker law.  They note that these nuclei are bluer
      than the background galaxy starlight, suggesting the presence of
      younger stars towards galactic centre {\bf AG mass
        threshold for this effect}. 

      In fact it is possible that the mass injection is dominated by a
      nuclear star cluster with a break radius on parsec
      scales, i.e. much smaller than our fiducial break
      radius of $r_b \sim 100$ pc. If the heating is sufficiently high moving the
      break radius inwards would merely change the outer gas density
      profile, without changing the stagnation radius in the flow and
      thus the accretion rate onto the black hole. Of course if one
      moves the break radius inside where the stagnation radius would
      be for large break radii, the stagnation radius would move
      inwards somewhat. However note the for high heating rate ($v_w
      \gsim 500 $ km/s and small black hole masses $\Mbh \lsim 10^7
      \Msun$), we predict stagnation radii smaller than a parsec.
