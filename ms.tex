...
\caption{\label{fig:miller} $\epsilon_X \Mdot c^2/L_{edd}$ predicted
  ...
  Figures~\ref{fig:vwSources} and~\ref{fig:eta}). Note that the star
  formation histories in \citet{MosterNaab+:2013a} are in terms of
  halo mass and for each $\Mbh$ we assign the halo mass, whose star
  formation history would produce a bulge consistent with the
  $\Mbh-M_{\rm bulge}$ relationship from \citet{McConnellMa:2013a}.
  We also show upper limits $\epsilon_X \Mdot c^2/L_{edd}$ from
