...
\subsubsection{Combined Heating Rate} The overall heating rate $\vwO$ has contributions from Type Ia SNes and main sequence winds.  The relative contributions will depend on both the age of the stellar population, $\tage$, and $\Mbh$.  This is illustrated in Figure~\ref{fig:vweff}. For black hole masses below $10^7 \Msun$ SNe
  ...
  \begin{figure}
    ...
    \caption{\label{fig:vweff} Combined heating rate $\vwO$ of the CNM in the parameter space of SMBH mass $\Mbh$ and age of the stellar population $t_{\star}$.  We assume that stellar winds contribute a constant heating rate $100$ km/s. The contribution from SNe Ia is calculated using equation (\ref{eq:vw_sne}). The contribution from SNe Ia is only added if the $r_s$ corresponding to the resulting $\vwO$ is outside $\rIa$.}%describe other cases here.
  \end{figure}
...
Figure 1 of \citealt{MillerGallo+:2014a} plots the observed
...
10^{-7}$, which gives comparable an x-ray luminosities comparable to
those in \citealt{MillerGallo+:2014a}.
%describe the quadratic procedure.

Our results are plotted in Figure~\ref{fig:bh_xray}. For each galaxy
in our sample we select the $\vwO$=200 km/s if $\rs<\rIa$
for this solution,  $\vwO$=500 km/s is $\rs>\rIa$ for this solution,
or exclude it.  $\vwO$ encapsulates heating SNe Ia heating, and it
would only make sense to include this heating if $\rs$>$\rIa$ as
discussed in Section~\ref{sec:heating}. At low $\Mbh$ we have
$\vwO$=200 km/s and at high $\Mbh$ we have $\vwO$=500 km/s.
Note that much of our sample is excluded in this Figure, since for
$\vwO$=200 km/s we would have $\rs>\rIa$ and four $\vwO$=500 km/s we
would have $\rs<\rIa$ (i.e. the relative locations of $\rs$ and $\rIa$
would be inconsistent with the assumed level of heating).

\begin{figure}
\includegraphics[width=\columnwidth]{bh_xray.eps}
\caption{\label{fig:bh_xray} Accretion power plotted as a function of 
  stellar mass for our solutions (points) with the trend of nuclear
  x-ray luminosity with stellar mass and the detection upper limit
  from Figure 1 of \citealt{MillerGallo+:2014a}. In the top panel we
  take the accretion power to be $\epsilon \Mdot c^2$, with
  $\epsilon=5\time 10^{-7}$ chosen to make our accretion powers
  comparable to the observed x-ray luminosities..}
\end{figure}
...
\label{sec:conduction}{
  ...
  \begin{enumerate}
    \item We find that $\eddr \propto \Mbh^{\alpha}$, where the value
      of $\alpha$ depends on the slope of the stellar density profile
      ($\alpha\simeq0.5,1$ for cusp and core galaxies respectively). 
