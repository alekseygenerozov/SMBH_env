...
behavior of the two vertically aligned green circles. These are two cusp
($\Gamma=0.8$) galaxies with the $\Mbh=10^7 \Msun$ and $v_w=300$ km/s
but with different break radii ($\rb=100, 400$ pc from top to
...
\begin{figure}
  ...
  \caption{\label{fig:stag} Stagnation radius $r_{s}$ in units of the
    ...
    cusp solutions which would be thermally unstable. The green circle
    with an upwards arrow corresponds to a lower bound--this is a
    model for where we were not able to obtain a converged value for
    the stagnation radius as it moves outwards when the outer boundary
    is moved outwards. The black curves correspond to the analytic
    prediction from equation (\ref{eq:rs2main}), with thick solid and
    dot-dashed curves calculated for parameters $(\Gamma=0.8,
    \densSlope\simeq 1)$ and $(\Gamma=0.1,\densSlope\simeq0.6)$,
    respectively. The thin black solid line corresponds to the
    simplified analytic result for $\rs$ from
    equation~\eqref{eq:stag_simple} (recall that $\rinf \simeq 14
    \Mbheight^{0.6} {\rm pc}$).}
\end{figure}
...
\begin{figure}
  ...
  \caption{\label{fig:mdot_mass} Accretion rate $\dot{M}/\dot{M}_{\rm
    ...
    would be thermally unstable.  The green circle with an upwards
    arrow corresponds to a lower bound--this is a model for where we
    were not able to obtain a converged value for the stagnation
    radius as it moves outwards when the outer boundary is moved
    outwards.  Thin solid and dot-dashed curves correspond to our
    analytic estimates of $\dot{M}/\dot{M}_{\rm edd}$
