...
Based on our numerical results and the recent work of \citet{McCourt+10}) we define thermal stability according to the criterion $\dot{q}_{\rm heat} > 10|\dot{q}_{\rm rad}|$ ($t_{\rm
...
\begin{tabular}{lccccccccc}
  ...
  $    10^{ 6 }$ & 300 & 100 & 0.12 & $ 5.1 \times 10^{ -5 }$ & 28 \\
... & 600 & 100 & $ 3.2 \times 10^{ -2 }$ & $ 1.0 \times 10^{ -5 }$ & $ 1.9 \times 10^{ 3 }$ \\
... & 1200 & 100 & $ 7.9 \times 10^{ -3 }$ & $ 1.8 \times 10^{ -6 }$ & $ 7.7 \times 10^{ 4 }$ \\
...
... & ... & 50 & 0.76 & $ 3.9 \times 10^{ -4 }$ & $\mathbf{ 8.0 \times 10^{ -2 }}$ \\
...
... & ... & 400 & 6.8 & $ 2.5 \times 10^{ -2 }$ & $\mathbf{ 3.1 \times 10^{ -4 }}$ \\
  \hline
\label{table:models}  
\end{tabular}
\begin{tablenotes}
...
Our analytic estimates accurately reproduce the numerical results in the high heating limit $v_{\rm w} \gg
\sigma_0$ ($r_{\rm s} \lesssim r_{\rm soi}$).  However, for sufficiently low $v_{\rm w}/\sigma_0$ the stagnation radius exceeds the analytic estimate, approaching the stellar break radius $r_b \gg r_{\rm soi}$.  This divergence occurs approximately when $\zeta = \sqrt{v_w^{2}+\sigma_0^{2}}/\sigma_0 < \zeta_c \propto (r_b/r_{\rm soi})^{0.5(\Gamma-1)}$ (eq.~[\ref{eq:zetacrit}], Appendix \ref{app:be}), and hence occurs at a higher value of $v_w/\sigma_0$ in the case of a core galaxies ($\Gamma = 0.8$) with large break radii.  This behavior explains the three core galaxies shown as green squares, which are vertically aligned at the same value of $v_{w}/\sigma$ (close to where the analytic estimate diverges) correspond to models with the same black hole mass ($\Mbh = 10^7 \Msun$) and heating rate ($v_w = 300$ km s$^{-1}$) but different values of the break radius: $r_{b} = $ 50, 100, and 400 pc (from top to bottom).  
