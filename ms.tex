...
\item Including the effects of energy transport by electron conduction results in only moderate changes to the key properties of the flow (e.g. stagnation radius, mass accretion rate, and thermal stability) for causal values of the conduction saturation parameter $\phi < 0.1$ (eq.~[\ref{eq:conduction}]).  

\item Radiative cooling has a more pronounced influence on the structure of the gas flow.  The approximate condition for cooling instabilities develop at all radii is that the radiative cooling rate exceed the external gas heating rate near the stagnation radius.  This condition is equivalent to the more commonly invoked $t_{\rm cool} \gg t_{\rm ff}$ criterion (e.g.~\citealt{McCourt+12}; \citealt{Li&Bryan14a}).   
...
We acknowledge helpful conversations with Greg Bryan, Jules Halpern, Kathryn Johnston, Jerry Ostriker, and Eliot
Quataert.  This work relied on Columbia University's Yeti compute
