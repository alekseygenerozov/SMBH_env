...
years.  To good approximation $\eta\simeq 0.02 (t_{\star}/t_h)^{-1.3}$ at time $t_{\star}$ following a burst of star formation (e.g., \citet{Ciotti+91}),\footnote{Note that 
\citet{Ciotti+91} gives the overall mass return rate from evolved
stars as a function of B-band luminosity. We give the mass return rate
per unit volume. However, our expressions are equivalent.} although $\eta$ will generally be higher in cases of ongoing star formation.  

Source terms $\propto q$ also appear in the momentum and entropy equations (eqs.~[\ref{eq:dvdt}] and [\ref{eq:dsdt}]) due to the fact that the isotropic mass injection in the rest frame of the SMBH
represents a source of momentum and energy relative to the mean flow (see also below).  Physically these terms result from the mismatch between the properties of virialized gas injected locally by stellar winds and the mean properties of the background flow.   As we shall discuss in $\S$, the cooling term $\propto q p/\rho$ represents an important stabilizing term in comparison to radiative cooling.  

The last term in equation (\ref{eq:dsdt}) accounts for heat transportation by electron conductivity, where $\kappa_{\rm eff}=\kappa_{\rm spitz}/(1+\psi)$ (\citealt{DaltonBalbus:1993a}) is the conducitivity and $\kappa_{\rm spitz} = \kappa_0 T^{5/2}$ is the classical \citet{Spitzer62} value ($\kappa_0\simeq 2\times 10^{-6}$ in cgs units).  The flux limitor $\psi = \kappa_{\rm spitz} \nabla T/(5\phi \rho c_s^3)$ saturates the conductive flux if the mean free path for electron coulomb scattering becomes comparable to the temperature length scale.  Here $c_s \equiv (kT/\mu m_p)^{1/2}$ is the isothermal sound speed and $\phi$ is an uncertain dimensionless constant, typically taken to be of order unity of less (we adopt $\phi = XXX$).  Even a weak magnetic field could in principle also suppress the conductivity perpendicular to the field.  However, the radially-decreasing temperature profiles of interest are susceptible to the magneto-thermal instability (\citealt{Balbus01}), the non-linear evolution of which is believed to result in a radially-directed field structure (\citealt{Parrish&Stone07}).

Following \citet{ShcherbakovWong+:2014a}, the term $\propto \vw^2 = \sigma(r)^2+v_{w,0}^2$ in the entropy equation accounts heating sources.  Here $\sigma(r) = \sqrt{GM_{\rm enc}/r}$ is the velocity dispersion of the stars, accounting for the minimum amount of shock heating from the stellar winds due to the random movement of stars with respect to each other at the characteristic velocity $\sim \sigma$.  This heating is present even for low wind velocities $\ll \sigma$, as characterize the majority of AGB mass loss.
The term $v_{w,0}^{2}$ parameterizes the contribution of additional sources of heating, including main sequence winds, millisecond pulsars, supernovae, AGN feedback, etc.  As discussed in $\S\ref{sec:heating}$, the value of $v_{w,0}$ is uncertain and will in general depend on the SMBH mass and the age of the stellar population.
...
different choices of $v_{\rm w,0} = 300, 600, 1200$ km s$^{-1}$ spanning the physically expected
range.  Although we are most interested in the steady-state CNM structure, the time-dependent equations are solved to avoid numerical issues that arise near the critical sonic points, of which there are up to two because of the supersonic inflow on small radial scales and a possible supersonic outflow on large scales.

Radiative cooling is not included in the entropy equation, although its effect is assessed critically in $\S\ref{sec:cooling}$.  Across much of the parameter space radiative cooling is subdominant compared to other sources of cooling (e.g., advection).  Radiative cooling cannot be neglected in some cases (if
$\vwO$ is low and/or $\eta$ is high), once it becomes important it leads to thermal instabilities, which our one-dimensional time-dependent modeal cannot account for anyways (e.g.~\citealt{McCourt+12}).  A chief goal of this work is instead to use our steady state solutions neglecting radiation to ascertain over what physical conditions cooling instabilities are likely to develop ($\S\ref{sec:cooling}$).
...
v_{\rm w} &=& v_{\rm w,0}\left(1+\frac{\sigma^{2}}{v_{\rm w,0}^{2}}\right)^{1/2} \nonumber \\
 &\approx& v_{\rm w,0}\left(1+0.14 \Mbheight^{0.43}\left[\frac{\vwO}{500\,{\rm km/s}}\right]^{-1}\right)^{1/2},
\end{eqnarray}
where $\vwO$ is a free parameter accounting for sources of heating beyond the baseline level produced by the stellar winds of old stars, and the final equality makes use of $M_{\bullet}-\sigma$ (eq.~[\ref{eq:Msigma}]).

\subsubsection{Properties at the Stagnation Radius}

First, continuity of the entropy derivative at the stagnation radius demands that the temperature at the stagnation radius is given by
\begin{align}
...
Hydrostatic equilibrium at the stagnation radius ($v = 0$) can be used to determine the stagnation radius
\begin{align}
...
\end{align}
where the numerical estimate assumes $n = 1$.  This expression is similar to that obtained by \citet{Volonteri+11} on heuristic grounds (their equation~[6]).  On the other hand, if wind heating is weaker ($v_{\rm w,0} \ll \sigma$), the stagnation radius moves to very large radii or may not exist at all.  
...
The density of gas at the stagnation radius $\rho(r_{\rm s})$ is more challenging to estimate.  We roughly estimate its value using equations (\ref{eq:stag_simple}) and (\ref{eq:mdot_analytic}) in conjunction with an alternative estimate of the mass accretion rate as the gaseous mass within the stagnation radius divided by the free-fall time $t_{\rm ff}(\rs) = (r_{\rm s}^{3}/GM_{\bullet})^{1/2}$, viz.
\begin{align}
...
Our expression for the mass accretion rate (eq.~[\ref{eq:mdot_analytic}]) is usefully compared to the traditional ``Bondi" accretion rate onto a point source from a surrounding medium of fixed sound speed $c_{\rm s} = (kT/\mu m_p)^{1/2}$ and
density (\citealt{Bondi52}):
\begin{align}
...
we note a similarity Bondi formula (eq.~[\ref{eq:bondi}]) with the exception that $\rb$ is replaced by $\rs$.  In fact, when the stagnation radius resides interior to the sphere of influence we have (eq.~[\ref{eq:stag_simple}])
\begin{align}
  \rs\approx\frac{7}{2}\frac{G \Mbh}{\vwO^2} \approx \frac{14}{3}\frac{GM_{\bullet}}{c_{s}^{2}} = \frac{14}{3}r_{\rm B},
  \label{eq:rs_simple}
\end{align}
where in the second line we have used equation (\ref{eq:Tanalytic}).  

\subsubsection{Importance of Heat Conduction}

Our analytic results neglect the effects of heat conduction near the stagnation radius, an assumption we now check.  The importance of conductive heating can be estimated by comparing its magnitude to that of ``wind" heating, viz.~
\begin{align}
  \left.\frac{\nabla\cdot(\kappa \nabla T)}{q v_{\rm
...
M_{\bullet,8}^{-0.5} v_{500}^{5.4} &  \text{unsaturated (cusp)}\\
 ...
 \label{eq:rhors}
\end{align}
where the second equality makes use of equations (\ref{eq:Tanalytic}), (\ref{eq:stag_simple}), and we have approximated $\nabla^{2} \sim 1/r_{\rm st}^{2}$, $\nabla \sim 1/r_{\rm st}$.  Because the stagnation radius is usually well inside the break radius, the stellar density is approximated as (eq.~[\ref{eq:rhostar}])
\begin{eqnarray}
 ...
 &\approx& \begin{cases}
  ...
  \label{eq:rhostarrs}
\end{eqnarray}
Even in the saturated heat flux case, heat conduction is at most moderately important for expected values of the saturation parameter $\phi < 1$, justifying its neglect in our analysis.  
