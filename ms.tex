...
\left.\frac{\dot{M}}{\dot{M}_{\rm edd}}\right|_{\rm max} &\approx& \frac{\eta M_{\bullet}}{\dot{M}_{\rm edd} t_{\rm h}}\left(\frac{R_{\rm Ia}}{R_{\rm soi}}\right)^{2-\Gamma} \approx \nonumber \\
 && \begin{cases}
    4.5 \times 10^{-4} M_{\bullet,8}^{-1.33}(\tau_{\star}/t_{\rm h})^{1.1}
   & \text{core} \\
    2.2 \times 10^{-4} \Mbheight^{-0.84}(\tau_{\star}/t_{\rm h})^{0.7}   & \text{cusp}.
  \end{cases}
  \label{eq:eddr_Ia}
\end{eqnarray}
{\bf AG: Fixed this expression. I will check again! Ia accretion rate should
  increase with time, as in the burst limit the Ia rate would decline
  with time, so the Ia radius would move outwards (need more volume to
  get a supernova per dynamical time.}
...
\caption{\label{fig:vwSources} Sources contributing to the heating rate of the CNM, $\vwO$, including stellar wind heating ({\it solid blue}), Ia supernovae ({\it dashed red}), millisecond pulsars ({\it solid orange}).  Each heating source varies with black hole mass $\Mbh$ due to the different average star formation history of galaxies of a given halo mass (see Appendix~\ref{app:windheat} for the detailed description).   We assume a stellar density power law slope of $\Gamma = 0.8$ in calculating $v_{w}^{\bullet \, 2}$ although the dependence on $\Gamma$ is weak {\bf BDM: just pick a value of Gamma!}.  The Ia heating rate is shown as a solid above the critical black hole mass for which $r_{\rm Ia} < r_{\rm s}$ (Fig.~\ref{fig:rs_rIa}), since this represents the condition for SN Ia to be well-approximated a continuously heating source. }
\end{figure}
