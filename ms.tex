...
\label{sec:results}
\subsection{Properties near the stagnation radius}
...
Our above analysis neglects heat conduction near the stagnation radius, an assumption we can now check.
...
M_{\bullet,8}^{-0.5} v_{500}^{5.4} &  \text{unsaturated (cusp)}\\
 ...
 \label{eq:rhors}
\end{align}
where in the second equality have made use of equations (\ref{eq:Tanalytic}), (\ref{eq:stag_simple}) and have approximated $\nabla^{2} \sim 1/r_{\rm st}^{2}$, $\nabla \sim 1/r_{\rm st}$.  Since the stagnation radius is usually well inside the break radius, the stellar density is approximated as (eq.~[\ref{eq:rhostar}])
\begin{eqnarray}
 ...
 &\approx& \begin{cases}
  ...
  \label{eq:rhostarrs}
\end{eqnarray}
Even in the case of saturated heat flux, we see that for expected values $\phi < 1$, heat condution is at most moderately important at the stagnation radius, justifying its neglect in our analysis.  
