...
radii outside of the black hole sphere of influence \citep{ChaeBernardi+:2014a}. For a given
$\Mbh$, we take $\sigma_0^2\approx 3 \sigma_{\Mbh}^2$, where
$\sigma_{\Mbh}$ is the velocity dispersion from the
\citet{McConnellMa+:2011a} $\Mbh-\sigma$ relation.  This heating is
...
energy $\sim \sigma^{2}$.  In fact, for core galaxies the specific
heating rate necessary to produce an unbound outflow can greatly
exceed $\sigma^{2}$ due to the additional gravitational potential of
stars outside the sphere of influence (Appendix~\ref{app:be}).
...
10^{-6}$) also necessarily has sufficient power to escape the nucler
...
early-type galaxies with on averager older stellar populations.
...
formation episodes (\citealt{Donas+07}).  Similiar cyclic AGN activity
...
qualitiatively similar to that the density profile we predict for core
...
SMBH mass $\Mbh\simeq 100\Msun$.  accroding to the mean relationship
