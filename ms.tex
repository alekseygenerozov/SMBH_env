...
\citealt{Kormendy+2013}), we adopt equation (\ref{eq:Msigma}) for lack
of an obviously better alternative.
...
We fix $\beta = 2$ because our results are not overly sensitive to the
properties of the gas flow on radial scales $\gtrsim r_b$ {\bf AG:
  beta=1 could be problematic for low heating rates}. The presence of
the break radius $r_{b}$ is, however, necessary to obtain a converged
steady state for some regions of our parameter space.  We consider
solutions calculated for up to four values of $\rb$: 50 pc, 100 pc,
200 pc, and 400 pc, motivated by the range of break radii from the
\citet{LauerFaber+:2007a} sample.\footnote{For the core galaxies the
  break radius follows the scaling relationship $\rb\sim 106 \,
...
\begin{cases}
  2.6 \times 10^{-2} \left(\eddr\right)^{0.9} & \eddr \lsim 10^{-4} \\
  2.6 \times 10^{-2} &  10^{-2} \lsim \eddr \lsim  10^{-4}\\
  0.1 \left(\eddr\right)^{0.58} &  10^{-2} \lsim \eddr \lsim  0.1\\
  0.1 & \eddr \gsim 0.1
\end{cases}
\label{eq:efficiency}
\end{align}
...
accretion flow.  Our conclusions are summarized as follows.
  ...
  \item Including the effects of heat transport by electron conduction
    ...
    model, given the inferred stellar rotation rates near the
    stagnation radius ($\S\ref{sec:rotation}$).
...
\item Low mass black holes grow in the low redshift universe over
  ...
  % \item Our calculation of $\Mdot$ agrees well with the one
  ...
  %   stagnation radius, $\rs$, which is within a factor of a few of
  %   the
