...
\Lambda(T)\simeq
  ...
  \label{eq:cooling2}
\end{align}
Note that at the stagnation radius equation (\ref{eq:cooling2}) is equivalent to the ratio of the cooling to free-fall timescale, i.e.
\begin{align}
\left. \frac{\tcool}{\tff}\right|_{\rs}&= \frac{9}{5} \left. \frac{q \vw^2/2}{\dot{q}_{\rm cool}}\right|_{\rs},
\end{align}
where we have used equations~(\eqref{eq:Tanalytic}) and (\eqref{eq:rhors2}). 
...
v_{\rm crit} \simeq
  ...
  \label{eq:cooling2}
\end{align}
In $\S\ref{sec:numerical}$ we will show that $v_{w} \gtrsim v_{\rm crit} \sim 200-300$ km s$^{-1}$ is generally a condition for stability at all radii, not just at the stagnation radius.  
