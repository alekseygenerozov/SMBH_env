...
\begin{figure}
  ...
  \caption{Minimum effective wind heating parameter required for
    ...
    {\bf AG update rb fit}. For $\zeta < \zeta_c$ the stagnation
    radius moves from inside the influence radius, out to the stellar
    break radius $r_b$ (Appendix \ref{app:be}); in practice renders
    the flow susceptible to thermal runaway, even if $v_{w} > v_{\rm
      TI}$.  For cusp galaxies the $v_w > v_{\rm TI}$ is usually the
    more strigent criterion for stability, while for core galaxies the
    $\zeta > \zeta_c$ is usually more relevant.}
\label{fig:TI}
\end{figure}
...
% \begin{threeparttable}
% \begin{minipage}{18cm}
% \caption{Comparison of CNM gas density at r=10$^{18}$ between our
%   model and existing constraints on the gas density on this scale for
%   the Milky Way from Chandra x-ray observations (first row) and from
%   modeling the radio light curve in the Swift J1644 transient (second row).}
% \begin{tabular}{lll}
%   {System} &  {Model} & {Comparison} \\
%   %\hline
%   Milky Way & 3 cm${-3}$ & 26 cm$^{-3}$\\
%   Swift J1644+57 & 5 cm$^{-3}$ & 0.8 cm$^{-3}$\\
%   %\hline
% \label{table:compare}  
% \end{tabular}
% % \begin{tablenotes}
% %   Model estimate has been corrected for the fact the analytic
% %   prescription underestimates gas density. This corresponds to model
% %   estimate for core galaxies.
% % \end{tablenotes}
% \end{minipage}
% \end{threeparttable}
% \end{table*}
...
\citet{Quataert2004} exceeds our estimate of $\dot{M}$ due to the
average SFH for the same $M_{\bullet}$ by almost two orders of
magnitude. This is because the mass source term in
\citet{Quataert2004} exceeds ours by several orders of magnitude.
...
compared to the result in \citet{Quataert2004} when the discreteness
