...
\begin{align} v_{w}^{\rm Ia} =\sqrt{\frac{2
      t_h R_{\rm Ia} E_{\rm Ia}}{\eta}} \label{eq:vw_sne}.
\end{align} 

The thermal energy injected by each SN Ia, $E_{\rm Ia} \simeq
...
accretion flow.  However, it is possible kinetic feedback from SMBH outflows could play an equal or greater role in regulating accretion.  Our simple
parameterization of kinetic feedback (eq.~[\ref{eq:vjet}]) assuming heating which is volumetrically uniform predicts an
