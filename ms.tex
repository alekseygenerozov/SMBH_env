...
results in at most an order unity correction for expected {\bf (AG:
  this sounds a bit too sharp perhaps replace with the word
  plausible. Perhaps we should also mention the phi=1 solutions have
  acausal conductivity (perhaps they are unphysical?).} values of
...
half-light radius and that $\lambda_R$ is with $R$ interior to this
point.  From equation (\ref{eq:rcirc}) we thus conclude that $r_{\rm
...
\item $^{(a)}$ Inner power-law index of stellar Nuker profile.
  ...
  parameter $\eta = 0.2$.  $^{(d)}$Ratio of cooling timescale to
  free-fall timescale at stagnation radius.  Thermally unstable
  ...
  saturatioon parameter $\phi=0.1$ (recall that we assume that the
...
\caption{\label{fig:bh_growth} {\it Top panel:} Critical values of the
  ...
  \citet{ShcherbakovWong+:2014a} (black diamonds). The top and bottom
  diamonds correspond to solution without and with conductivity
  respectively. Note that \citet{ShcherbakovWong+:2014a} do not
  include saturation of the thermal conductivity in their model {\it Bottom
