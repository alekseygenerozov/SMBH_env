...
critical threshold \be \zeta\equiv \sqrt{1 + \frac{v_w}{\sigma_0}^2} <
...
core galaxies $r_{b}/r_{\rm soi} \approx 7.6M_{\bullet,8}^{-0.2}$ {\bf
AG: this is the second time I have caught this, is there any
particular reason you think this should be +0.2 not -0.2} which
...
$\alpha = -0.2$ (see also \citep{Zhang+09, Pellegrini10, Gallo+10}).
...
This prediction is in better with the observed dependence of $\langle
L_{X}/L_{\rm edd} \rangle$ in elliptical galaxies
(Fig.~\ref{fig:miller}, green line), hinting that accretion regulation
by kinetic feedback may serve as an alternative explanation for X-ray
luminosities of elliptical galaxies.  However, our assumption that
kinetic outflows heat the gas uniformly in volume is somewhat
...
environment.  It is furthermore unclear whether our steady-state model
can be applied at all, because black hole feedback depends on the
central accretion rate, which (unlike stellar feedback) does not
respond instantaneously to changes in the thermodynamic properties of
the gas on larger scales.
...
\item Galaxies with $10^{6}M_{\odot} \lesssim \Mbh \lesssim 10^8
  \Msun$ with star formation histories matching the average values
  derived from the cosmological simulations of
  \citet{MosterNaab+:2013a} possess an effective heating rate $v_w\sim
  1000$ km s$^{-1}$, relatively independent of SMBH mass
  (Fig.~\ref{fig:vwSources}).  This results in a prediction for the
  average Eddington ratio $\eddr \propto \Mbh^{\alpha}$ with
  $\alpha\lsim 0.5 \, (0.8)$ for core (cusp) galaxies which is tension
  with the trend of measured nuclear X-ray luminosities of early-type
  galaxies (e.g., \citealt{Miller+15}), which instead find that
  $\eddr$ increases towards smaller $\Mbh$.  However, high X-ray
  radiative efficiencies are required for these galaxies to be
  consistent with thermally-stable accretion, instead suggesting that
  (at least at low black hole masses), the possibility of two branches
  of accretion states.


    % However, it is important to note that the inflow rate on large
    % scales (which we calculate) may differ from
