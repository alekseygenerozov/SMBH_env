...
\begin{abstract}
  ...
  hot gas in the nominally quiescent galactic nuclei for a range of
  ...
  black hole accretion rate, $\dot{M}$, as a function of $M_{\bullet}$ and the gas heating efficiency, the latter of which is
  ...
  of a single burst of star formation and for the average
  star formation history, as predicted by cosmological simulations, of galaxies hosting black holes of a given mass. The measured nuclear X-ray luminosities, $L_{X}$, of nearby quiescent galactic nuclei and low luminosity AGN are combined with our results for $\dot{M}(M_{\bullet})$ to address whether the nuclei of most nearby galaxies are consistent with accreting in a steady-state, thermally-stable manner.  The maximum accretion rate of a thermally-stable flow is too low to explain the short average growth times of low mass black holes in the local
  ...
  $L_{\rm X}-M_{\bullet}$ correlation, as well as applications to other topics, such as the predicted diversity of the
  circumnuclear media encountered by relativistic outflows from
...
We include a few solutions with cooling (marked with a $\dagger$) and
conductivity (marked with a $\ddagger$) Table \ref{table:models} to
assess whether it is valid to neglect these effects. As expected
solutions for which $\dot{q}_{\rm heat}/\dot{q}_{\rm rad}\gg 1$ are
unaffected when radiative cooling is included (see the solution with
$\Mbh=10^{8} \Msun, v_w=600 {\rm km/s and} \gamma=0.8$. However, we
find that as we decrease the heating further, to cooling begins to
become more important (see $\Mbh=10^{8} \Msun, v_w=450 {\rm km/s and}
\gamma=0.8$). Overall, we find radiative cooling has negligible effect
on cusp solutions $v_w\geq 600 km/s$ in and core solutions with
$v_w=1200$ km/s {\bf AG for the purposes of description it may be
  simpler to just rerun all of our models with radiative cooling}. 

We re-ran all cusps ($\Gamma=0.8$) with $v_w=300$ km/s and
all cores ($\Gamma=0.1$) with $v_w=600 $ km/s with radiative cooling
for three different values of the mass input parameter $\eta$--0.02,
0.2, and 0.6.  Note that $\eta=0.02$ is the minimum expected physical
mass input parameter. Solutions which were unstable for all values of
$\eta$ are marked with a $\mathbf{TI}$ {\bf solution that were
  unstable for only some choices of eta}.
...
$\zeta = \sqrt{v_w^{2}+\sigma_0^{2}}/\sigma_0$, with different colors
...
occurs approximately when $\zeta =
\sqrt{v_w^{2}+\sigma_0^{2}}/\sigma_0 < \zeta_c \propto (r_b/r_{\rm
  soi})^{0.5(\Gamma-1)}$ (eq.~[\ref{eq:zetacrit}], Appendix
\ref{app:be}), and hence occurs at a higher value of $\zeta$ in the
case of core galaxies ($\Gamma = 0.8$) with large break radii. 

As $\zeta$ approached $\zeta_c$, the results the location of the
stagnation radius will depend on the location of the break radius
$\rb$.  This explains the behavior of the two core galaxies, shown as
