...
where for the numerical estimate we adopt values of $n \approx 1$ and $n \approx 0.5$ for core ($\Gamma = 0.1$) and cusp ($\Gamma = 0.8$) galaxies, respectively, so chosen to match the results of our numerical calculations (Fig.~\ref{fig:stag}).  This expression is similar to that obtained by \citet{Volonteri+11} on more heuristic grounds (their eq.~6). 
