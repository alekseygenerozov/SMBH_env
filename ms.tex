...
More precisely we find that there is a critical $\zeta_c$
($\zeta\equiv \sqrt{v_w^2+\sigma_0^2}/\sigma_0$), such that if
$\zeta\lsim\zeta_c$, then $\rs\simeq\rb$. For $\rb\gsim\rsoi$ we find 

\begin{align}
\zeta_c\simeq \left(\frac{\rb}{\rsoi}\right)^{0.5(1-\Gamma)},
\end{align}

as described in Appendix~\ref{app:be}. 
...
cent.  Note the sensitive inverse dependence of the accretion rate on
...
One observes a similarity between equation (\ref{eq:mdot_gas}) and the
Bondi formula (eq.~[\ref{eq:bondi}]) if $r_{\rm B}$ is replaced by
$\rs$.  Indeed, when the stagnation radius resides interior to the
sphere of influence we find that (eq.~[\ref{eq:stag_simple}])
\begin{align}
...
\frac{\dot{M}_{\rm TI}}{\dot{M}_{\rm edd}} \simeq
  ...
  \label{eq:Mdotmax}
\end{align}
which is derived using equation (\ref{eq:eddr_analytic}) to calculate
$\dot{M}(v_{\rm TI})$.
