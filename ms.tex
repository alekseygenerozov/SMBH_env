...
smaller galaxies will on average have more recent star formation, resulting in energetic young stellar winds and supernovae which dominate the energy budget, resulting in a high heating rate of $\vwO \sim 1000 $ km s$^{-1}$.  Massive galaxies, on the other hand, will on average have older stellar populations, with their heating rates dominated by Ia supernovae (on large radial scales) and SMBH feedback.  We estimate the average value of $\vwO$ as a function of SMBH mass for each heating source by calculating its value using the average cosmic star formation histories of \citet{MosterNaab+:2013a}.
Appendix \ref{app:windheat} describes how the average star formation history is used to determine the stellar wind heating $v_{\rm  w}^{\star}$ and mass return parameter $\eta$ (Fig.~\ref{fig:eta}; top panel).  Star formation histories are also convolved with the Ia DTD distribution to determine the SN Ia heating.  

Figure~\ref{fig:vwSources} shows $\vwO(M_{\bullet})$ from each heating source (stars, MSPs, SNe Ia, and black hole feedback) calculated for the average star formation history of galaxies containing a given black hole mass.  Again, Ia heating is suppressed when the Ia radius exceeds the nominal stagnation radius, with $r_{\rm Ia}/r_{\rm s}$ shown in the bottom panel of Figure ~\ref{fig:eta}.  MSP heating is again found to be negligible for all black hole masses.  The younger stellar populations characterizing galaxies with $\Mbh\lsim 5\times 10^{8} \Msun$ results in stellar wind and Ia heating being dominant.  For $\Mbh\gsim 5\times 10^{8} \Msun$, the lack of younger stellar populations reduces the role of stellar wind heating, with Compton heating playing a more important role.  Again, solid and dashed lines show black hole masses for which the flow is thermally stable and thermally unstable, respectively, as shown in the bottom panel.  
