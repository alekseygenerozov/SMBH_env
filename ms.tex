...
\begin{abstract}
  ...
  winds, while gas is heated primarily by stellar winds, supernovae,
  ...
  conditions under which radiative instabilities develop, separately
  in the case of a single burst of star formation and for the average
  star formation history predicted by cosmological simulations for
  galaxies hosting black holes of a given mass.  By combining a sample
...
operating at higher SMBH masses and accretion rates in high redshift
quasars.  One key question is whether SMBHs grow primarily by the
...
\item Type Ia SNe only provide a continuous heating source exterior to the Ia radius $\rIa$ (eq.~[\ref{eq:rIa}]) where the time between subsequent SNe exceeds the dynamical timescale.  Non-Ia heating due to the average SFH results in $\rs <\rIa$, except for the most massive galaxies (Fig.~\ref{fig:vwSources}).  However, if $\rs$ does approach or exceed $\rIa$, then the SN Ia heating is usually sufficiently large to prevent $\rs$ from greatly exceeding $\rIa$.  SNe Ia thus regulates the SMBH accretion rate below the value $\dot{M}_{\rm Ia}$ (eq.~[\ref{eq:eddr_Ia}]), except possibly in high mass elliptical galaxies with large break radii and $\zeta(v_w^{\rm Ia}) \gtrsim \zeta_c$. 
