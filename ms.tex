...
Finally, we note that some low mass galaxies with $\Mbh\lsim 2 \times 10^{8} \Msun$ possess nuclear star clusters (\citealt{GrahamSpitler:2009a}), which are not accounted for by our simple parametrization of the stellar density.  In such cases gas and energy injection could be dominated by the cluster itself, i.e. concentrated within its own pc-scale ``break radius" which is much smaller than the outer break in the older stellar population on much larger scales.  If the extent of the cluster is known, however, our model can be fully generalized to this case.  Furthermore, for high heating rates the stagnation radius and concomitant accretion rate are not sensitive to the location of the break radius.
