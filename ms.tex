...
and $\Gamma=0.1$.  Since our results are most sensitive to the
properties of the gas flow on radial scales $\ll r_b$, we fix $\beta =
2$ (a typical value). In any case, the flow inside $\rb$ is
insensitive the outer power law slope of the stellar density
profile. However, the presence of $\rb$ is necessary for to obtain a
...
and 400 pc. These choices are motivated by the observed break radii in
the \citet{LauerFaber+:2007a} sample. For the core galaxies
($\Gamma \le$0.3) in this sample the break radius follows the scaling
relationship $\rb\sim 106 \, (\Mbheight)^{0.39}$ pc, with scatter of
approximately one dex. Most the cusp ($\Gamma \ge0.5$) galaxies have
$\rb$ between 100 and 1000 pc, and there is not trend with
$\Mbh$. {\bf AG: In practice when it is clear the rb will not have a
...
\label{eq:Tanalytic}
\end{align}
where $v_{500} \equiv \tilde{v}_{w}/500$ km s$^{-1}$.
...
\end{eqnarray}
where we have made use of equation (\ref{eq:stag_simple}), assuming that $v_{w} \gg \sigma$.  The resulting Eddington ratio $\eddr$ is given by 
\begin{eqnarray}
...
T^{7/2}}{r_{\rm s}^{2}\eta \rho_{\star}(r_{\rm s}) \tilde{v}_{w}^{2}(1+\phi)}
\nonumber \\ &\sim {\rm min}
  \begin{cases}
  10 \eta_{0.02}^{-1}
...
\sigma,     
  ...
  \label{eq:cooling2}
\end{align}
Our numerical results in $\S\ref{sec:numerical}$ demonstrate that $\tilde{v}_{w} \gtrsim v_{\rm crit} \sim 200-300$ km s$^{-1}$ is generally a condition for stability at all radii, not just at the stagnation radius.  The inclusion of $\sigma$ in equation (\ref{eq:cooling2}) results because the top two lines apply only for $\tilde{v}_{w} > \sigma$ since their derivation assumed the existence of a stagnation radius given by equation (\ref{eq:rs_simple}).
...
\hline
{$M_{\bullet,8}$} & {$v_{w}$} & {$\gamma$} & {$r_b$} & {$\dot{M}/\dot{M}_{\rm Edd}$} & {$t_{\rm cool}/t_{\rm ff} (r_s)$} & {comments}\\
\hline
0.01 & 300.0 & 0.8 & N/A & $ 3.2 \times 10^{ -4 }$ & 4.7 & rb in fact
unresolved \\
0.01 & 600.0 & 0.8 & N/A & $ 5.5 \times 10^{ -5 }$ & $ 3.2 \times
10^{ 2 }$ &  rb in fact unresolved\\
0.01 & 1200.0 & 0.8 & N/A & $ 9.4 \times 10^{ -6 }$ & $ 1.5 \times
...
1 & 300.0 & 0.8 & 100.0 & $ 1.6 \times 10^{ -2 }$ & $ 5.6 \times 10^{
  -2 }$ &  TI\\
1 & 600.0 & 0.8 & 100.0 & $ 7.3 \times 10^{ -4 }$ & 21 &  \\
1 & 1200.0 & 0.8 & 100.0 & $ 9.9 \times 10^{ -5 }$ & $ 1.5 \times 10^{
  3 }$ &  \\
...
%  -3 }$ &  TI/convergence\\
1 & 300.0 & 0.8 & 400.0 &  &  &  TI/run unsuccessful\\
0.1 & 300.0 & 0.8 & 50.0 & $ 2.0 \times 10^{ -3 }$ & 0.46 &  \\
0.1 & 300.0 & 0.8 & 200.0 & $ 5.3 \times 10^{ -3 }$ & 0.12 &  TI/convergence\\
%0.1 & 300.0 & 0.8 & 300.0 & $ 7.2 \times 10^{ -3 }$ & $ 6.7 \times
%10^{ -2 }$   & TI/convergence  \\
0.1 & 300.0 & 0.8 & 400.0 & $ 8.9 \times 10^{ -3 }$ & $ 4.7 \times
10^{ -2 }$ & TI/convergence \\
0.01 & 1200.0 & 0.1 & N/A & $ 1.5 \times 10^{ -6 }$ & $ 4.1 \times
10^{ 4 }$ &  rb is unsresolved\\
0.1 & 1200.0 & 0.1 & N/A & $ 9.5 \times 10^{ -6 }$ & $ 5.7 \times 10^{
  3 }$ &  tried run in which rb=100 pc was resolvedwhat would happen if rb=400 pc?\\

1 & 1200.0 & 0.1 & 100.0 & $ 6.0 \times 10^{ -5 }$ & $ 8.6 \times 10^{ 2 }$ &  \\
1 & 1200.0 & 0.1 & 300.0 & $ 1.9 \times 10^{ -4 }$ & $ 1.3 \times 10^{
  2 }$ &  \\
0.01 & 1200.0 & 0.1 & 100.0 & $ 1.5 \times 10^{ -6 }$ & $ 4.1 \times 10^{ 4 }$ &  \\
0.1 & 1200.0 & 0.1 & 100.0 & $ 9.5 \times 10^{ -6 }$ & $ 5.7 \times 10^{ 3 }$ &  \\
1 & 1200.0 & 0.1 & 100.0 & $ 6.0 \times 10^{ -5 }$ & $ 8.6 \times 10^{ 2 }$ &  \\
1 & 1200.0 & 0.1 & 50.0 & $ 4.9 \times 10^{ -5 }$ & $ 1.2 \times 10^{ 3 }$ &  \\
1 & 1200.0 & 0.1 & 200.0 & $ 1.0 \times 10^{ -4 }$ & $ 3.4 \times 10^{ 2 }$ &  convergence\\
1 & 1200.0 & 0.1 & 300.0 & $ 1.9 \times 10^{ -4 }$ & $ 1.3 \times 10^{ 2 }$ &  convergence\\
1 & 1200.0 & 0.1 & 400.0 & $ 1.8 \times 10^{ -2 }$ & 0.37 &  TI/convergence\\
...
$v_{w}$.  Cusp and core galaxies are marked with square and
...
\caption{\label{fig:vwSources} $\vwO$ for stellar winds ({\it solid
  ...
  $\Gamma$, since $v_{w}^{\bullet \, 2}$ depends on
