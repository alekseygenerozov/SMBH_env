...
Figure~\ref{fig:vwSourcesImp} shows the contributions of various heating sources as a
function of time $\tau_{\star}$ after a burst of star formation for a
black hole masses of $\Mbh = 10^{6} \Msun$.  These heating
rates are calculated using the stellar wind heating $v_{\rm
  w}^{\star}$ (Fig.~\ref{fig:vwImp}) and mass return parameter $\eta$
(Fig.~\ref{fig:eta}), the latter of which affects the other sources of
feedback, including AGN ($v_{\rm w}^{\bullet}$; eqs.~[\ref{eq:vC}],
[\ref{eq:vjet}]), millisecond pulsars ($v_{w}^{\rm msp}$;
eq.~[\ref{eq:vmsp}]), and Ia supernovae ($v_{w}^{\rm Ia}$;
eq.~[\ref{eq:vIa}]).  The Ia heating rate also depends on a
convolution of the star formation history and the Ia delay-time
distribution (eq.~[\ref{eq:DTD}]).  At very early times ($\tau_{\star}
\lesssim XXX$), stellar wind heating dominates, but at later times
Compton heating dominates.  MSP heating is again found to be
negligible at all black hole masses.  Epochs of thermally-unstable
accretion are shown as a dashed line, indicating that Compton heating
becomes important only during the phase of unstable accretion.

From Figure~\ref{fig:vwSourcesImp} we conclude that thermally unstable accretion is the
norm.  However, a single burst of star formation does not describe the
