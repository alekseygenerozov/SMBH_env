...
\begin{cases}
  2.6 \times 10^{-2} \left(\frac{\eddr}{10^{-4}}\right)^{0.9} & \eddr \lsim 10^{-4} \\
  2.6 \times 10^{-2} &  10^{-2} \lsim \eddr \lsim  10^{-4}\\
  0.1 \left(\frac{\eddr}{0.1}\right)^{0.58} &  10^{-2} \lsim \eddr \lsim  0.1\\
  0.1 & \eddr \gsim 0.1
\end{cases}
\label{eq:efficiency}
\end{align}
...
approximately reproduce the solid line in Figure 6
in \citet{Sharma+2007}, which assumes 33\% of the viscous dissipation
heats the electrons. For $\eddr\gsim 0.1$, $\epsilon_{rad}$ should saturate
to 0.1, hence the fourth line. The third line is a power law which
connects $\epsilon_{\rm rad} (\eddr=0.01) = 0.026$ to $\epsilon_{\rm
  rad} (\eddr=0.1) = 0.1$.
...
$\alpha \Mdot$.  Taking $epsilon=\alpha \epsilon_{\rm rad}$ for the
overall efficiency with $\alpha=0.1$ and $\epsilon_{\rm rad}$ from
equation~\eqref{eq:efficiency} we obtain {\bf AG note the change in
  the description of the efficiency}, 
...
\caption{\label{fig:miller} Average nuclear X-ray luminosity, $\langle
  ...
  (\alpha/0.1) \epsilon_{rad} (\eddr) $, where $\epsilon_{\rm rad}$ is
  given by equation~\eqref{eq:efficiency}.  Shown for comparison are
  the X-ray luminosities calculated for the maximum thermally-stable
  accretion rate ({\it dashed orange line}; eq.~[\ref{eq:Mdotmax}]);
  the SN Ia-regulated accretion rate ({\it dashed teal line};
